...
TEST_F(ConnectionPoolIntegrationTest, BasicPoolFunctionality) {
    ...
    ASSERT_TRUE(conn_result2) << "Failed to get second connection from pool: " << conn_result2.error().message;
...
TEST_F(ConnectionPoolIntegrationTest, WithConnectionHelper) {
    ...
    auto verify_result = pool->with_connection([inserted_id](auto conn) {
        auto select_result = conn->execute_raw(
            "SELECT value FROM test_pool WHERE id = ?",
        ...
        if (rows.size() != 1) {
            return std::make_pair(false, std::string("Expected 1 row, got ") + std::to_string(rows.size()));
        }
...
TEST_F(ConnectionPoolIntegrationTest, ConcurrentAccess) {
    ...
    for (int t = 0; t < thread_count; ++t) {
        threads.emplace_back([this, t, &result_mutex, &inserted_ids, &success_count, &error_count, iterations_per_thread]() {
            for (int i = 0; i < iterations_per_thread; ++i) {
                ...
                auto result = pool->with_connection([t, i](auto conn) {
                    ...
                    auto insert_result = conn->execute_raw(
                        "INSERT INTO test_pool (value) VALUES (?) RETURNING id",
                        {value}
                    );
...
TEST_F(ConnectionPoolIntegrationTest, PoolUnderLoad) {
    ...
    EXPECT_EQ(connection_count - (connection_count / 2), pool->active_connections())
        << "Expected " << (connection_count - (connection_count / 2)) << " active connections after returns";
...
TEST_F(ConnectionPoolIntegrationTest, ConnectionValidation) {
    ...
    ASSERT_TRUE(new_conn_result) << "Failed to get new connection after broken one: " << new_conn_result.error().message;
    
    auto new_conn = *new_conn_result;
    
    // Verify the new connection works
    auto query_result = new_conn->execute_raw("SELECT 1 as test");
    ASSERT_TRUE(query_result) << "New connection failed basic query: " << query_result.error().message;
...
TEST_F(ConnectionPoolIntegrationTest, PoolTransactions) {
    ...
    ASSERT_TRUE(verify_result) << "Failed to verify transaction data: " << verify_result.error().message;
    ...
    auto rollback_result = pool->with_connection([](auto conn) {
        ...
        // Insert a record
        auto insert = conn->execute_raw("INSERT INTO test_pool (value) VALUES ('should_be_rolled_back')");
        if (!insert) {
            conn->rollback_transaction();
            return false;
        }
    ...
    ASSERT_TRUE(rollback_result) << "Failed to execute rollback test: " << rollback_result.error().message;
