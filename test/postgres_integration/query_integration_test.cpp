...
TEST_F(QueryIntegrationTest, BasicSelect) {
  ...
  auto query = select(product.id, product.name, product.price).from(product).order_by(product.id);
...
TEST_F(QueryIntegrationTest, OrderByClause) {
  ...
  auto asc_query =
      select(product.name, product.price).from(product).order_by(product.price);  // Ascending
...
TEST_F(QueryIntegrationTest, LimitAndOffset) {
  ...
  // Test with limit
  auto limit_query = select(product.id, product.name).from(product).order_by(product.id).limit(3);
  ...
  auto offset_query =
      select(product.id, product.name).from(product).order_by(product.id).limit(2).offset(3);
  ...
  auto just_offset_query =
      select(product.id, product.name).from(product).order_by(product.id).offset(5);
...
TEST_F(QueryIntegrationTest, AggregateFunctions) {
  ...
  // Test COUNT
  auto count_query = select_expr(count(product.id)).from(product).where(product.category_id == 1);
  ...
  // Test SUM
  auto sum_query = select_expr(sum(order.total)).from(order).where(order.customer_id == 1);
  ...
  EXPECT_DOUBLE_EQ(1039.97, *sum_row[0].get<double>(0))
      << "Expected sum of 1039.97 for customer 1's orders";

  // Test AVG
  auto avg_query = select_expr(avg(product.price)).from(product);
  ...
  EXPECT_NEAR(expected_avg, *avg_row[0].get<double>(0), 0.01)
      << "Expected average price around 410.82";

  // Test MIN/MAX
  auto min_max_query = select_expr(min(product.price), max(product.price)).from(product);
...
TEST_F(QueryIntegrationTest, AliasesAndExpressions) {
  ...
  auto query = select_expr(as(count_all(), "total_count"), as(sum(product.price), "total_price"),
                           as(avg(product.price), "avg_price"), as(min(product.price), "min_price"),
  ...
  EXPECT_NEAR(total, *row[0].get<double>("total_price"), 0.01)
      << "Expected total price around 2464.94";

  double avg = total / 6.0;
  EXPECT_NEAR(avg, *row[0].get<double>("avg_price"), 0.01)
      << "Expected average price around 410.82";
...
TEST_F(QueryIntegrationTest, GroupBy) {
  ...
  auto query = select_expr(order.customer_id, as(count_all(), "order_count"),
...
TEST_F(QueryIntegrationTest, Having) {
  ...
  auto query = select_expr(product.category_id, as(count_all(), "product_count"),
