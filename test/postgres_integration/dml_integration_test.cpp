...
TEST_F(DmlIntegrationTest, InsertOperations) {
  ...
  auto select_all_categories =
      select(category.id, category.name).from(category).order_by(category.id);
...
TEST_F(DmlIntegrationTest, UpdateOperations) {
  ...
  // Verify update
  auto select_updated = select(category.description).from(category).where(category.id == 1);
  ...
  // ASSERT_TRUE(result) << "Failed to select multiple updated categories: " <<
  ...
  // ASSERT_TRUE(result) << "Failed to select subquery updated category: " <<
  // result.error().message;
...
TEST_F(DmlIntegrationTest, DeleteOperations) {
  ...
  // Delete a single row
  auto delete_single = delete_from(category).where(category.id == 4);
  ...
  auto select_all = select(count_all()).from(category);
  ...
  auto check_deleted = select(category.id).from(category).where(category.id == 4);
  ...
  // result = conn->execute(select_all);
  // ASSERT_TRUE(result) << "Failed to count categories after subquery delete: " <<
  // result.error().message;
  ...
  // result = conn->execute(select_all);
  // ASSERT_TRUE(result) << "Failed to count categories after returning delete: " <<
  // result.error().message;
...
TEST_F(DmlIntegrationTest, TransactionSupport) {
  ...
  ASSERT_TRUE(conn_result2) << "Failed to begin second transaction: "
  ...
  ASSERT_TRUE(select_result2) << "Failed to select after commit: "
                              << select_result2.error().message;
  ...
  ASSERT_TRUE(conn_result3) << "Failed to begin third transaction: "
                            << conn_result3.error().message;
  ...
  ASSERT_TRUE(rollback_result2) << "Failed to rollback after error: "
                                << rollback_result2.error().message;
  ...
  ASSERT_TRUE(select_result3) << "Failed to select products after rollback: "
                              << select_result3.error().message;
