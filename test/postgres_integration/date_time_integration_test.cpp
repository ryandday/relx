...
protected:
  ...
  void TearDown() override { cleanupTables(); }
...
TEST_F(DateTimeIntegrationTest, BasicDateTimeOperations) {
  ...
  for (const auto& row : rows) {
    ...
    if (event_date.has_value()) {
      ...
      EXPECT_GT(time_t_val, 1640995200)
          << "Event date should be after 2022-01-01";  // Unix timestamp for 2022-01-01
    }
  }
...
TEST_F(DateTimeIntegrationTest, DateExtractionFunctions) {
  ...
  auto query = select_expr(
                   events.name, as(extract("year", events.event_date), "event_year"),
...
TEST_F(DateTimeIntegrationTest, DateArithmeticOperations) {
  ...
  auto query = select_expr(events.name, events.event_date,
...
TEST_F(DateTimeIntegrationTest, CurrentDateTimeFunctions) {
  ...
  auto query = select_expr(as(current_date(), "current_date"),
                           as(current_timestamp(), "current_timestamp"), as(now(), "now"));
  ...
  // Verify basic format
  EXPECT_GE(current_date_val->length(), 10)
      << "Current date should be at least 10 characters (YYYY-MM-DD)";
  EXPECT_GE(current_timestamp_val->length(), 19)
      << "Current timestamp should be at least 19 characters";
...
TEST_F(DateTimeIntegrationTest, DateHelperFunctions) {
  ...
  auto query = select_expr(employees.name, employees.birth_date,
  ...
  if (birth_date.has_value()) {
    EXPECT_EQ(static_cast<int>(birth_date->year()), 1990)
        << "Birth year from chrono type should be 1990";
    EXPECT_EQ(static_cast<unsigned>(birth_date->month()), 3u)
        << "Birth month from chrono type should be 3";
    EXPECT_EQ(static_cast<unsigned>(birth_date->day()), 22u)
        << "Birth day from chrono type should be 22";
  }
...
TEST_F(DateTimeIntegrationTest, TimeZoneOperations) {
  ...
  auto query = select_expr(time_zones.zone_name, time_zones.utc_time, time_zones.local_time,
                           time_zones.offset_hours,
                           // Calculate the difference in seconds between UTC and local time
                           as(date_diff("second", time_zones.utc_time, time_zones.local_time),
                              "time_diff_seconds"))
                   .from(time_zones)
...
TEST_F(DateTimeIntegrationTest, DateOperationsWithNulls) {
  ...
  auto query = select_expr(events.name, events.end_time,
                           as(extract("hour", events.start_time),
                              "start_hour")  // Always extract from non-null column
                           )
                   .from(events)
                   .order_by(events.name);  // Order by string column
  ...
  for (const auto& row : rows) {
    auto name = row.get<std::string>(0);
    auto end_time = row.get<std::optional<std::chrono::system_clock::time_point>>(
