...
protected:
  ...
  void insert_test_data() {
    ...
    auto check_category = select(category.id, category.name).from(category).where(category.id == 4);
    ...
    // Insert products
    auto insert_products =
        insert_into(product)
            .columns(product.id, product.category_id, product.name, product.description,
    ...
    auto check_customer = select(customer.id, customer.name).from(customer).where(customer.id == 4);
    ...
    auto insert_orders = insert_into(order)
                             .columns(order.id, order.customer_id, order.product_id, order.quantity,
    ...
    auto check_orders = select(order.id).from(order).where(order.customer_id == 4);
...
TEST_F(JoinIntegrationTest, InnerJoin) {
  ...
  auto query = select(order.id, customer.name, product.name, order.total)
                   .from(order)
...
TEST_F(JoinIntegrationTest, LeftJoin) {
  ...
  auto left_join_query = relx::query::select(customer.id, customer.name,
  ...
  ASSERT_TRUE(found_alice_with_null_order)
      << "Alice should have a NULL order_id in LEFT JOIN results";

  // Now test the query builder version
  auto query = select(customer.id, customer.name, relx::as(order.id, "order_id"))
                   .from(customer)
                   .left_join(order, customer.id == order.customer_id)
  ...
  ASSERT_TRUE(found_alice_in_builder)
      << "Alice should have a NULL order_id in query builder LEFT JOIN results";
}
...
TEST_F(JoinIntegrationTest, RightJoin) {
  ...
  auto direct_sql =
      "SELECT c.id, c.name, p.id AS product_id, p.name AS product_name FROM products p "
      "RIGHT JOIN categories c ON p.category_id = c.id "
      "ORDER BY c.id, p.id";

  auto right_join_query = relx::query::select(category.id, category.name,
  ...
  for (size_t i = 0; i < rows.size(); i++) {
    ...
    if (cat_id && *cat_id == 4) {
      ...
      if (product_id && !product_id->has_value() && product_name && !product_name->has_value()) {
  ...
  ASSERT_TRUE(found_empty_category_direct)
      << "Empty Category should have NULL product values in RIGHT JOIN results";

  // Now test the query builder version
  auto query = select(category.id, category.name, relx::as(product.id, "product_id"),
                      relx::as(product.name, "product_name"))
                   .from(product)
                   .right_join(category, product.category_id == category.id)
                   .order_by(category.id)
  ...
  ASSERT_TRUE(found_empty_category_builder)
      << "Empty Category should have NULL product values in query builder RIGHT JOIN results";
}
...
TEST_F(JoinIntegrationTest, FullOuterJoin) {
  ...
  auto direct_sql =
      "SELECT c.id AS customer_id, c.name, o.id AS order_id, o.status FROM customers c "
      "FULL OUTER JOIN orders o ON c.id = o.customer_id "
      "ORDER BY c.id, o.id";

  auto full_outer_join_query = relx::query::select(relx::as(customer.id, "customer_id"),
                                                   customer.name, relx::as(order.id, "order_id"),
                                                   order.status)
                                   .from(customer)
  ...
  for (size_t i = 0; i < rows.size(); i++) {
    ...
    if (cust_id && *cust_id == 4) {
      ...
      if (order_id && !order_id->has_value() && status && !status->has_value()) {
        auto name = row.get<std::string>(1);
        ASSERT_TRUE(name.has_value());
        EXPECT_EQ("Alice Brown", *name);
  ...
  ASSERT_TRUE(found_alice_direct)
      << "Alice should have NULL order values in FULL OUTER JOIN results";

  // Now test the query builder version
  auto query = select(relx::as(customer.id, "customer_id"), customer.name,
                      relx::as(order.id, "order_id"), order.status)
                   .from(customer)
                   .full_join(order, customer.id == order.customer_id)
                   .order_by(customer.id)
                   .order_by(order.id);
  ...
  ASSERT_TRUE(found_alice_builder)
      << "Alice should have NULL order values in query builder FULL OUTER JOIN results";
}
...
TEST_F(JoinIntegrationTest, ComplexJoins) {
  ...
  auto query = select(relx::as(order.id, "order_id"), relx::as(customer.name, "customer_name"),
                      relx::as(product.name, "product_name"),
                      relx::as(category.name, "category_name"), order.quantity, order.total,
                      order.status)
                   .from(order)
                   .join(customer, order.customer_id == customer.id)
                   .join(product, order.product_id == product.id)
...
//     auto& rows = *result;
//     ASSERT_EQ(6, rows.size()) << "Expected 6 product pairs (2 in Electronics, 2 in Clothing, 2 in
//     Books)";
...
//     result = conn->execute_raw(customers_with_orders.to_sql(),
//     customers_with_orders.bind_params()); ASSERT_TRUE(result) << "Failed to execute EXISTS
//     subquery: " << result.error().message;
...
TEST_F(JoinIntegrationTest, JoinWithConditions) {
  ...
  auto query = select(order.id, customer.name, product.name, product.price, order.quantity,
                      order.total)
                   .from(order)
                   .join(customer, order.customer_id == customer.id)
                   .join(product, order.product_id == product.id)
...
TEST_F(JoinIntegrationTest, JoinWithAggregates) {
  ...
  auto query = select(category.id, category.name, as(count(product.id), "product_count"),
