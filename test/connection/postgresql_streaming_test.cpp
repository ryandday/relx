...
protected:
  void SetUp() override {
    ...
    std::string conn_str = "host=localhost port=5434 dbname=relx_test user=postgres password=postgres";
  ...
  void setup_test_data() {
    ...
    for (int i = 1; i <= 1000; ++i) {
      ...
      ASSERT_TRUE(insert_result) << "Failed to insert user " << i << ": " << insert_result.error().message;
    }
  }
...
TEST_F(PostgreSQLStreamingTest, BasicStreamingFunctionality) {
  ...
  connection::PostgreSQLStreamingSource source(*connection, 
    "SELECT id, name, email, age FROM users ORDER BY id LIMIT 10");
...
TEST_F(PostgreSQLStreamingTest, StreamingWithParameters) {
  ...
  connection::PostgreSQLStreamingSource source(*connection, 
    "SELECT id, name, age FROM users WHERE age >= ? AND age <= ? ORDER BY id",
    {"25", "35"});
  auto init_result = source.initialize();
  ASSERT_TRUE(init_result) << "Failed to initialize streaming: " << init_result.error().message;
...
TEST_F(PostgreSQLStreamingTest, StreamingWithSchemaIntegration) {
  ...
  // Create and initialize the streaming source
  connection::PostgreSQLStreamingSource source(*connection, 
    "SELECT id, name, email, age FROM users ORDER BY id LIMIT 5");
  auto init_result = source.initialize();
  ASSERT_TRUE(init_result) << "Failed to initialize streaming: " << init_result.error().message;
...
TEST_F(PostgreSQLStreamingTest, StreamingWithNullValues) {
  ...
  ASSERT_TRUE(insert_result) << "Failed to insert user with NULL: " << insert_result.error().message;
  
  // Create source to stream the row with NULL
  connection::PostgreSQLStreamingSource source(*connection, 
    "SELECT name, email FROM users WHERE name = 'NullUser'");
  auto init_result = source.initialize();
  ASSERT_TRUE(init_result) << "Failed to initialize streaming: " << init_result.error().message;
...
TEST_F(PostgreSQLStreamingTest, PerformanceComparison) {
  ...
  // Calculate timing
  auto regular_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_regular - start_regular).count();
  auto streaming_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_streaming - start_streaming).count();
