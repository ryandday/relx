...
protected:
  ...
  void clean_test_table() {
    ...
    if (connect_result) {
      ...
      const std::vector<std::string> statement_names = {
          "insert_statement", "insert_typed_statement", "statement1", "insert_stmt", "update_stmt",
          "select_stmt"};
      ...
      if (result) {
        for (const auto& row : *result) {
          ...
          if (stmt_name) {
            ...
            if (std::find(statement_names.begin(), statement_names.end(), *stmt_name) !=
                statement_names.end()) {
              try {
  ...
  void create_test_table(relx::connection::PostgreSQLConnection& conn) {
    ...
    const std::vector<std::string> statement_names = {"insert_statement", "insert_typed_statement",
                                                      "statement1",       "insert_stmt",
                                                      "update_stmt",      "select_stmt"};
