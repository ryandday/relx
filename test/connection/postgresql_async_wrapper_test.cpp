...
//         auto result = co_await conn.connect("host=localhost port=54321 dbname=nonexistent
//         user=postgres password=postgres"); EXPECT_FALSE(result); EXPECT_FALSE(conn.is_open());
//     });
// }
...
TEST_F(PostgresqlAsyncWrapperTest, BasicTransaction) {
  ...
  run_test([&]() -> asio::awaitable<void> {
    ...
    auto create = co_await conn.query(
        "CREATE TABLE transaction_test (id SERIAL PRIMARY KEY, value TEXT)");
    EXPECT_TRUE(create);
    ...
    // Insert a row
    auto insert = co_await conn.query(
        "INSERT INTO transaction_test (value) VALUES ($1) RETURNING id", {"test_value"});
    EXPECT_TRUE(insert);
...
TEST_F(PostgresqlAsyncWrapperTest, AutoRollbackOnClose) {
  ...
  run_test([&]() -> asio::awaitable<void> {
    ...
    auto create = co_await conn.query(
        "CREATE TABLE auto_rollback_test (id SERIAL PRIMARY KEY, value TEXT)");
    EXPECT_TRUE(create);
