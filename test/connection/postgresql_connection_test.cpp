...
TEST_F(PostgreSQLConnectionTest, TestMoveOperations) {
  ...
  relx::PostgreSQLConnection conn3(
      "host=localhost port=5434 dbname=nonexistent user=postgres password=postgres");
...
TEST_F(PostgreSQLConnectionTest, TestTransactionBasics) {
  ...
  ASSERT_TRUE(insert_result) << "Failed to insert in transaction: "
  ...
  auto verify_result = conn.execute_raw("SELECT COUNT(*) FROM users WHERE name = $1",
                                        {"TransactionTest"});
...
TEST_F(PostgreSQLConnectionTest, TestTransactionRollback) {
  ...
  auto verify_in_tx_result = conn.execute_raw("SELECT COUNT(*) FROM users WHERE name = $1",
                                              {"RollbackTest"});
  ...
  auto verify_after_rollback = conn.execute_raw("SELECT COUNT(*) FROM users WHERE name = $1",
                                                {"RollbackTest"});
...
TEST_F(PostgreSQLConnectionTest, TestTransactionIsolationLevels) {
  ...
  std::vector<relx::IsolationLevel> levels = {
      relx::IsolationLevel::ReadUncommitted, relx::IsolationLevel::ReadCommitted,
      relx::IsolationLevel::RepeatableRead, relx::IsolationLevel::Serializable};

  for (auto level : levels) {
    ASSERT_TRUE(conn.begin_transaction(level))
        << "Failed to begin transaction with isolation level: " << static_cast<int>(level);
    ASSERT_TRUE(conn.in_transaction());

    // Run a simple query within the transaction
    auto query_result = conn.execute_raw("SELECT 1");
    ASSERT_TRUE(query_result) << "Query failed in transaction with isolation level: "
...
TEST_F(PostgreSQLConnectionTest, TestBooleanColumn) {
  ...
  ASSERT_TRUE(true_result) << "Failed to query with boolean condition: "
                           << true_result.error().message;
  ...
  ASSERT_TRUE(false_result) << "Failed to query with boolean condition: "
                            << false_result.error().message;
  ...
  auto update_result = conn.execute_raw(
      "UPDATE users SET active = NOT active WHERE name = $1 RETURNING active", {"TrueAsT"});
