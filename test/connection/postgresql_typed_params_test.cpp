...
TEST_F(PostgreSQLTypedParamsTest, TestBasicTypedParameters) {
    ...
    auto result = conn.execute_typed(
        "INSERT INTO typed_params_test (int_val, float_val, text_val, bool_val, nullable_val) VALUES (?, ?, ?, ?, ?)",
        42,                // int
        3.14159,           // float
        "Hello, world!",   // string
        true,              // bool
...
TEST_F(PostgreSQLTypedParamsTest, TestMixedTypes) {
    ...
    auto update_result = conn.execute_typed(
        "UPDATE typed_params_test SET int_val = ?, text_val = ? WHERE bool_val = ?",
        64,
        "Updated text",
        false
    );
...
TEST_F(PostgreSQLTypedParamsTest, TestCustomTypeConversion) {
    ...
    auto result = conn.execute_typed(
        "INSERT INTO typed_params_test (int_val, text_val) VALUES (?, ?)",
        42,
