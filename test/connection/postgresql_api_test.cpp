...
TEST_F(PostgreSQLApiTest, TestInsertAndSelect) {
  ...
  auto insert2 = conn.execute(relx::query::insert_into(p)
                                  .columns(p.name, p.description, p.price, p.in_stock, p.category)
  ...
  auto select_result = conn.execute(
      relx::query::select(p.id, p.name, p.price, p.category).from(p).order_by(p.id));
  ...
  auto filtered_result = conn.execute(relx::query::select(p.id, p.name, p.price)
                                          .from(p)
                                          .where(p.category == "Electronics")
                                          .order_by(p.price));

  ASSERT_TRUE(filtered_result) << "Filtered select query failed: "
                               << filtered_result.error().message;
...
TEST_F(PostgreSQLApiTest, TestUpdate) {
  ...
  // Update the product
  auto update_result = conn.execute(relx::query::update(p)
  ...
  auto verify_result = conn.execute(
      relx::query::select(p.id, p.name, p.price, p.category).from(p).where(p.id == 1));
...
TEST_F(PostgreSQLApiTest, TestDelete) {
  ...
  auto insert3 = conn.execute(relx::query::insert_into(p)
                                  .columns(p.name, p.description, p.price, p.in_stock, p.category)
                                  .values("Product 3", "Description 3", 30.99, true, "Category A"));
  ASSERT_TRUE(insert3) << "Failed to insert test data 3: " << insert3.error().message;

  // Verify we have 3 products
  auto count_result1 = conn.execute(relx::query::select(relx::query::count(p.id)).from(p));
  ...
  auto delete_result = conn.execute(relx::query::delete_from(p).where(p.id == 2));
  ASSERT_TRUE(delete_result) << "Delete query failed: " << delete_result.error().message;

  // Verify we now have 2 products
  auto count_result2 = conn.execute(relx::query::select(relx::query::count(p.id)).from(p));
  ...
  auto delete_result2 = conn.execute(relx::query::delete_from(p).where(p.category == "Category A"));
  ASSERT_TRUE(delete_result2) << "Delete query failed: " << delete_result2.error().message;

  // Verify we now have 0 products
  auto count_result3 = conn.execute(relx::query::select(relx::query::count(p.id)).from(p));
...
TEST_F(PostgreSQLApiTest, TestTransactionsWithApi) {
  ...
  auto verify_result = conn.execute(
      relx::query::select(p.id, p.name).from(p).where(p.category == "Transaction"));
  ASSERT_TRUE(verify_result) << "Verification query failed: " << verify_result.error().message;
  ASSERT_EQ(1, verify_result->size());
  ...
  auto verify_in_tx = conn.execute(
      relx::query::select(relx::query::count(p.id)).from(p).where(p.category == "Rollback"));
  ASSERT_TRUE(verify_in_tx) << "In-transaction verification failed: "
                            << verify_in_tx.error().message;
  ...
  ASSERT_TRUE(verify_after_rollback)
      << "Post-rollback verification failed: " << verify_after_rollback.error().message;
...
TEST_F(PostgreSQLApiTest, TestPostgreSQLReturningClause) {
  ...
  ASSERT_TRUE(multi_insert_result)
      << "Multi-row insert with RETURNING failed: " << multi_insert_result.error().message;
  ASSERT_EQ(2, multi_insert_result->size()) << "Expected 2 rows from multi-row INSERT RETURNING";
  ASSERT_EQ(2, multi_insert_result->column_count())
      << "Expected 2 columns from multi-row INSERT RETURNING";
  ...
  EXPECT_TRUE(std::find(returned_names.begin(), returned_names.end(), "Bulk Product 1") !=
              returned_names.end());
  EXPECT_TRUE(std::find(returned_names.begin(), returned_names.end(), "Bulk Product 2") !=
              returned_names.end());
