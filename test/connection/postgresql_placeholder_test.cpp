...
TEST_F(PostgreSQLPlaceholderTest, NoPlaceholders) {
    EXPECT_EQ("SELECT * FROM users", 
              convert_placeholders("SELECT * FROM users"));
}
...
TEST_F(PostgreSQLPlaceholderTest, QuestionMarkInStringLiteral) {
    ...
    EXPECT_EQ("SELECT * FROM users WHERE question = 'Why?' AND answer = 'Because!' AND id = $1", 
              convert_placeholders("SELECT * FROM users WHERE question = 'Why?' AND answer = 'Because!' AND id = ?"));
}

TEST_F(PostgreSQLPlaceholderTest, EscapedQuotesInStringLiteral) {
    // Test escaped quotes within string literals
    EXPECT_EQ("SELECT * FROM users WHERE name = 'John''s question?' AND id = $1", 
...
TEST_F(PostgreSQLPlaceholderTest, ComplexMixedCase) {
    ...
    std::string sql = "SELECT \"table?name\", 'string?literal', ? FROM users WHERE id = ? AND name = 'What''s up?'";
    std::string expected = "SELECT \"table?name\", 'string?literal', $1 FROM users WHERE id = $2 AND name = 'What''s up?'";
...
TEST_F(PostgreSQLPlaceholderTest, EscapedQuotesEdgeCases) {
    ...
    EXPECT_EQ("'Don''t ask ''why?'' twice'", 
              convert_placeholders("'Don''t ask ''why?'' twice'"));
