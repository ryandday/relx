...
TEST(ExpectedUtilityTest, ValueOrThrowCausesException) {
  ...
  relx::connection::ConnectionError error{.message = "Test connection error", .error_code = 123};
  ...
  EXPECT_THROW({ relx::value_or_throw(error_result); }, relx::RelxException);

  // Test with context
  EXPECT_THROW({ relx::value_or_throw(error_result, "Custom context"); }, relx::RelxException);
}

TEST(ExpectedUtilityTest, ValueOrThrowQueryErrorFormatted) {
  relx::query::QueryError query_error{.message = "SQL syntax error"};
  std::expected<int, relx::query::QueryError> result = std::unexpected<relx::query::QueryError>(
      query_error);

  EXPECT_THROW({ relx::value_or_throw(result); }, relx::RelxException);
}
...
TEST(ExpectedUtilityTest, ValueOrThrowResultErrorFormatted) {
  ...
  std::expected<int, relx::result::ResultError> result = std::unexpected<relx::result::ResultError>(
      result_error);
...
TEST(ExpectedUtilityTest, ValueOrThrowRvalueSupport) {
  ...
  EXPECT_EQ(relx::value_or_throw(std::expected<int, relx::connection::ConnectionError>{123}), 123);

  // Test with rvalue error
  EXPECT_THROW(
      {
        relx::value_or_throw(std::expected<int, relx::connection::ConnectionError>{
            std::unexpected<relx::connection::ConnectionError>(relx::connection::ConnectionError{
                .message = "Connection failed", .error_code = 500})});
      },
      relx::RelxException);
}
...
TEST(ExpectedUtilityTest, ThrowIfFailedVoidResultType) {
  ...
  EXPECT_NO_THROW({ relx::throw_if_failed(success_void); });

  // Test with error
  relx::connection::ConnectionError error{.message = "Void operation failed", .error_code = 999};
  ...
  EXPECT_THROW({ relx::throw_if_failed(error_void); }, relx::RelxException);
  ...
  EXPECT_THROW(
      {
        relx::throw_if_failed(std::expected<void, relx::connection::ConnectionError>{
            std::unexpected<relx::connection::ConnectionError>(relx::connection::ConnectionError{
                .message = "Rvalue void operation failed", .error_code = 888})});
      },
      relx::RelxException);
}