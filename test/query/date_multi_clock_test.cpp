...
TEST_F(MultiClockDateTest, SystemClockBasicFunctions) {
  // Test date_diff
  auto diff_query =
      select_expr(date_diff("day", system_table.timestamp, current_date())).from(system_table);

  EXPECT_EQ(
      diff_query.to_sql(),
      "SELECT (CURRENT_DATE::date - system_clock_events.timestamp::date) FROM system_clock_events");
  EXPECT_TRUE(diff_query.bind_params().empty());

  // Test extract
  auto extract_query = select_expr(extract("year", system_table.timestamp)).from(system_table);

  EXPECT_EQ(extract_query.to_sql(),
            "SELECT EXTRACT(year FROM system_clock_events.timestamp) FROM system_clock_events");

  // Test date arithmetic
  auto arithmetic_query =
      select_expr(date_add(system_table.timestamp, interval("1 year"))).from(system_table);

  EXPECT_EQ(arithmetic_query.to_sql(),
            "SELECT (system_clock_events.timestamp + INTERVAL '1 year') FROM system_clock_events");

  // Test with optional column
  auto optional_query = select_expr(year(system_table.optional_timestamp)).from(system_table);

  EXPECT_EQ(
      optional_query.to_sql(),
      "SELECT EXTRACT(year FROM system_clock_events.optional_timestamp) FROM system_clock_events");
}
...
TEST_F(MultiClockDateTest, SteadyClockBasicFunctions) {
  // Test date_diff
  auto diff_query =
      select_expr(date_diff("hour", steady_table.timestamp, now())).from(steady_table);

  EXPECT_EQ(diff_query.to_sql(), "SELECT EXTRACT(EPOCH FROM (NOW() - "
                                 "steady_clock_events.timestamp))/3600 FROM steady_clock_events");
  EXPECT_TRUE(diff_query.bind_params().empty());

  // Test extract
  auto extract_query = select_expr(extract("month", steady_table.timestamp)).from(steady_table);

  EXPECT_EQ(extract_query.to_sql(),
            "SELECT EXTRACT(month FROM steady_clock_events.timestamp) FROM steady_clock_events");

  // Test date arithmetic with operator
  auto arithmetic_query =
      select_expr(steady_table.timestamp + interval("6 months")).from(steady_table);

  EXPECT_EQ(
      arithmetic_query.to_sql(),
      "SELECT (steady_clock_events.timestamp + INTERVAL '6 months') FROM steady_clock_events");

  // Test with optional column
  auto optional_query = select_expr(month(steady_table.optional_timestamp)).from(steady_table);

  EXPECT_EQ(
      optional_query.to_sql(),
      "SELECT EXTRACT(month FROM steady_clock_events.optional_timestamp) FROM steady_clock_events");
}
...
TEST_F(MultiClockDateTest, HighResClockBasicFunctions) {
  ...
  EXPECT_EQ(diff_query.to_sql(), "SELECT EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - "
                                 "high_res_clock_events.timestamp))/60 FROM high_res_clock_events");
  EXPECT_TRUE(diff_query.bind_params().empty());

  // Test extract
  auto extract_query = select_expr(extract("day", high_res_table.timestamp)).from(high_res_table);

  EXPECT_EQ(extract_query.to_sql(),
            "SELECT EXTRACT(day FROM high_res_clock_events.timestamp) FROM high_res_clock_events");

  // Test date truncation
  auto trunc_query = select_expr(date_trunc("hour", high_res_table.timestamp)).from(high_res_table);

  EXPECT_EQ(
      trunc_query.to_sql(),
      "SELECT DATE_TRUNC('hour', high_res_clock_events.timestamp) FROM high_res_clock_events");

  // Test with optional column
  auto optional_query = select_expr(day(high_res_table.optional_timestamp)).from(high_res_table);

  EXPECT_EQ(optional_query.to_sql(),
            "SELECT EXTRACT(day FROM high_res_clock_events.optional_timestamp) FROM "
            "high_res_clock_events");
}
...
TEST_F(MultiClockDateTest, HelperFunctionsAllClocks) {
  ...
  auto system_helpers = select_expr(as(year(system_table.timestamp), "sys_year"),
  ...
  auto high_res_helpers = select_expr(as(day_of_week(high_res_table.timestamp), "hr_dow"),
...
TEST_F(MultiClockDateTest, MixedClockOperations) {
  ...
  auto mixed_query = select_expr(mixed_table.id, as(year(mixed_table.system_time), "sys_year"),
  ...
  auto complex_mixed =
      select_expr(as(date_add(mixed_table.system_time, interval("1 year")), "sys_plus_year"),
...
TEST_F(MultiClockDateTest, WhereClausesWithDifferentClocks) {
  ...
  auto system_where = select(system_table.id, system_table.name)
                          .from(system_table)
                          .where((year(system_table.timestamp) >= 2020) &&
  ...
  auto steady_where = select(steady_table.id, steady_table.name)
                          .from(steady_table)
                          .where((hour(steady_table.timestamp) >= 9) &&
...
TEST_F(MultiClockDateTest, OrderByWithDifferentClocks) {
  ...
  auto mixed_order = select(mixed_table.id)
                         .from(mixed_table)
                         .order_by(desc(year(mixed_table.system_time)),
...
TEST_F(MultiClockDateTest, GroupByWithDifferentClocks) {
  ...
  auto grouped_query = select_expr(year(mixed_table.system_time), month(mixed_table.steady_time),
                                   as(count_all(), "event_count"),
                                   as(min(day(mixed_table.high_res_time)), "min_day"),
                                   as(max(hour(mixed_table.optional_system)), "max_hour"))
                           .from(mixed_table)
                           .group_by(year(mixed_table.system_time), month(mixed_table.steady_time))
                           .having(count_all() > 5);
...
TEST_F(MultiClockDateTest, ComplexNestedMultiClock) {
  ...
  auto complex_query =
      select_expr(
          mixed_table.id,
          as(year(date_add(start_of_year(mixed_table.system_time), interval("6 months"))),
             "mid_year_sys"),
          as(month(mixed_table.steady_time + interval("3 months")), "future_month_steady"),
          as(day(date_trunc("month", mixed_table.high_res_time)), "month_start_day_hr"),
          as(hour(date_sub(mixed_table.optional_system, interval("2 hours"))), "past_hour_opt_sys"))
          .from(mixed_table)
          .where((year(mixed_table.system_time) >= year(mixed_table.steady_time)) &&
  ...
  EXPECT_EQ(complex_query.to_sql(),
            "SELECT mixed_clock_events.id, "
            "EXTRACT(year FROM (DATE_TRUNC('year', mixed_clock_events.system_time) + INTERVAL '6 "
            "months')) AS mid_year_sys, "
            "EXTRACT(month FROM (mixed_clock_events.steady_time + INTERVAL '3 months')) AS "
            "future_month_steady, "
            "EXTRACT(day FROM DATE_TRUNC('month', mixed_clock_events.high_res_time)) AS "
            "month_start_day_hr, "
            "EXTRACT(hour FROM (mixed_clock_events.optional_system - INTERVAL '2 hours')) AS "
            "past_hour_opt_sys "
            "FROM mixed_clock_events "
            "WHERE (((EXTRACT(year FROM mixed_clock_events.system_time) >= EXTRACT(year FROM "
            "mixed_clock_events.steady_time)) AND "
            "(EXTRACT(month FROM mixed_clock_events.steady_time) = EXTRACT(month FROM "
            "mixed_clock_events.high_res_time))) AND "
            "(EXTRACT(day FROM mixed_clock_events.system_time) <= EXTRACT(day FROM "
            "mixed_clock_events.high_res_time)))");
}
...
TEST_F(MultiClockDateTest, TypeSafetyCompilation) {
  ...
  auto valid_system = select_expr(date_diff("day", system_table.timestamp, current_date()),
  ...
  auto valid_steady = select_expr(date_diff("hour", steady_table.timestamp, now()),
  ...
  auto valid_optional = select_expr(year(system_table.optional_timestamp),
...
TEST_F(MultiClockDateTest, SpecialHelpersWithDifferentClocks) {
  ...
  auto system_helpers = select_expr(as(age_in_years(system_table.timestamp), "age_years"),
  ...
  auto steady_helpers = select_expr(as(age_in_years(steady_table.timestamp), "steady_age"),
  ...
  auto high_res_helpers = select_expr(as(age_in_years(high_res_table.timestamp), "hr_age"),
