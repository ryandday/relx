...
struct arithmetic_test_table {
    ...
    relx::schema::column<arithmetic_test_table, "optional_price", std::optional<double>> optional_price;
    relx::schema::column<arithmetic_test_table, "optional_quantity", std::optional<int>> optional_quantity;
};
...
TEST_F(ArithmeticTest, BasicColumnAddition) {
    auto query = relx::query::select_expr(table.id + table.quantity)
        .from(table);
...
TEST_F(ArithmeticTest, BasicColumnSubtraction) {
    auto query = relx::query::select_expr(table.price - table.discount)
        .from(table);
...
TEST_F(ArithmeticTest, BasicColumnMultiplication) {
    auto query = relx::query::select_expr(table.price * table.quantity)
        .from(table);
...
TEST_F(ArithmeticTest, ColumnWithValueAddition) {
    auto query = relx::query::select_expr(table.price + 10.5)
        .from(table);
...
TEST_F(ArithmeticTest, ValueWithColumnAddition) {
    auto query = relx::query::select_expr(5 + table.id)
        .from(table);
...
TEST_F(ArithmeticTest, ColumnWithValueSubtraction) {
    auto query = relx::query::select_expr(table.price - 5.0)
        .from(table);
...
TEST_F(ArithmeticTest, ValueWithColumnSubtraction) {
    auto query = relx::query::select_expr(100 - table.id)
        .from(table);
...
TEST_F(ArithmeticTest, ColumnWithValueMultiplication) {
    auto query = relx::query::select_expr(table.price * 1.2)
        .from(table);
...
TEST_F(ArithmeticTest, ValueWithColumnMultiplication) {
    auto query = relx::query::select_expr(2.5 * table.price)
        .from(table);
...
TEST_F(ArithmeticTest, ColumnWithValueDivision) {
    auto query = relx::query::select_expr(table.price / 2.0)
        .from(table);
...
TEST_F(ArithmeticTest, ValueWithColumnDivision) {
    auto query = relx::query::select_expr(100.0 / table.price)
        .from(table);
...
TEST_F(ArithmeticTest, OptionalColumnAddition) {
    auto query = relx::query::select_expr(table.id + table.optional_id)
        .from(table);
...
TEST_F(ArithmeticTest, OptionalColumnWithValue) {
    auto query = relx::query::select_expr(table.optional_price * 1.5)
        .from(table);
...
TEST_F(ArithmeticTest, ArithmeticWithAlias) {
    ...
    auto query = relx::query::select_expr(
        table.id,
        relx::query::as(total, "total_price")
    ).from(table);
    
    auto sql = query.to_sql();
    auto params = query.bind_params();
    
    EXPECT_EQ(sql, "SELECT test_table.id, (test_table.price * test_table.quantity) AS total_price FROM test_table");
    EXPECT_TRUE(params.empty());
}

TEST_F(ArithmeticTest, ArithmeticInOrderBy) {
    auto query = relx::query::select(table.id, table.price)
        .from(table)
        .order_by(table.price * table.quantity);
    
    auto sql = query.to_sql();
    auto params = query.bind_params();
    
    EXPECT_EQ(sql, "SELECT test_table.id, test_table.price FROM test_table ORDER BY (test_table.price * test_table.quantity)");
    EXPECT_TRUE(params.empty());
}
...
TEST_F(ArithmeticTest, ArithmeticWithMixedTypes) {
    ...
    auto query = relx::query::select_expr(mixed)
        .from(table);
...
TEST_F(ArithmeticTest, MultipleSimpleArithmeticColumns) {
    ...
    EXPECT_EQ(sql, "SELECT (test_table.price + ?), (test_table.quantity * ?), (test_table.id - ?) FROM test_table");
...
TEST_F(ArithmeticTest, ArithmeticWithOptionalAndRegularColumns) {
    auto query = relx::query::select_expr(
        table.optional_price + table.price,
    ...
    EXPECT_EQ(sql, "SELECT (test_table.optional_price + test_table.price), (test_table.id * test_table.optional_quantity) FROM test_table");
    EXPECT_TRUE(params.empty());
}
...
TEST_F(ArithmeticTest, ArithmeticWithIntegerLiterals) {
    ...
    EXPECT_EQ(sql, "SELECT (test_table.id + ?), (test_table.quantity - ?), (test_table.id * ?), (test_table.quantity / ?) FROM test_table");
...
TEST_F(ArithmeticTest, ArithmeticWithFloatLiterals) {
    ...
    EXPECT_EQ(sql, "SELECT (test_table.price + ?), (test_table.discount - ?), (test_table.price * ?), (test_table.discount / ?) FROM test_table");
    ASSERT_EQ(params.size(), 4);
