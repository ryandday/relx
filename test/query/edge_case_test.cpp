...
TEST(EdgeCaseTest, SpecialCharactersInStrings) {
  ...
  auto query = relx::query::select(u.id, u.name).from(u).where(u.name == special_chars);
...
TEST(EdgeCaseTest, UnicodeStrings) {
  ...
  auto query = relx::query::select(u.id, u.name).from(u).where(u.name == unicode_string);
...
TEST(EdgeCaseTest, VeryLongStrings) {
  ...
  auto query = relx::query::select(u.id, u.name).from(u).where(u.bio == long_string);
...
TEST(EdgeCaseTest, BooleanValues) {
  ...
  auto query_false = relx::query::select(u.id, u.name).from(u).where(u.is_active == false);
...
TEST(EdgeCaseTest, ExtremeDateValues) {
  ...
  auto query_min = relx::query::select(p.id, p.title).from(p).where(p.created_at == min_date);

  auto query_max = relx::query::select(p.id, p.title).from(p).where(p.created_at == max_date);
...
TEST(EdgeCaseTest, NestedLogicalOperators) {
  ...
  auto query = relx::query::select(u.id, u.name)
                   ...
                   .where(u.is_active == true &&
                          ((u.age < 30 || u.age > 60) &&
                           (u.login_count > 5 || relx::query::is_not_null(u.bio))));
...
//         .join(pt, relx::query::on(relx::query::to_expr(p.id) ==
//         relx::query::to_expr(pt.post_id))) .join(t,
//         relx::query::on(relx::query::to_expr(pt.tag_id) == relx::query::to_expr(t.id)))
