...
TEST(ConditionTest, LogicalAnd) {
  ...
  std::string expected_sql =
      "SELECT users.id, users.name FROM users WHERE ((users.age >= ?) AND (users.is_active = ?))";
  EXPECT_EQ(query.to_sql(), expected_sql);
...
TEST(ConditionTest, LogicalOr) {
  ...
  std::string expected_sql =
      "SELECT users.id, users.name FROM users WHERE ((users.age < ?) OR (users.age >= ?))";
  EXPECT_EQ(query.to_sql(), expected_sql);
...
TEST(ConditionTest, ComplexLogicalExpression) {
  ...
  auto query = relx::query::select(u.id, u.name)
                   .from(u)
                   .where((u.age >= 18) && (u.is_active == true || u.login_count > 10));

  // Note: testing exact parentheses nesting is implementation-dependent
  std::string expected_sql = "SELECT users.id, users.name FROM users WHERE ((users.age >= ?) AND "
                             "((users.is_active = ?) OR (users.login_count > ?)))";
  EXPECT_EQ(query.to_sql(), expected_sql);
...
TEST(ConditionTest, StringNotLike) {
  ...
  std::string expected_sql =
      "SELECT users.id, users.name FROM users WHERE (NOT users.email LIKE ?)";
  EXPECT_EQ(query.to_sql(), expected_sql);
...
TEST(ConditionTest, NotInList) {
  ...
  // Use operator! directly to negate the condition
  auto query =
      relx::query::select(u.id, u.email).from(u).where(!(relx::query::in(u.age, age_strings)));

  std::string expected_sql =
      "SELECT users.id, users.email FROM users WHERE (NOT users.age IN (?, ?, ?))";
  EXPECT_EQ(query.to_sql(), expected_sql);
...
TEST(ConditionTest, Between) {
  ...
  auto query =
      relx::query::select(u.id, u.name).from(u).where(relx::query::between(u.age, "18", "65"));

  std::string expected_sql =
      "SELECT users.id, users.name FROM users WHERE users.age BETWEEN ? AND ?";
  EXPECT_EQ(query.to_sql(), expected_sql);
...
TEST(ConditionTest, NotBetween) {
  ...
  std::string expected_sql =
      "SELECT users.id, users.name FROM users WHERE (NOT users.age BETWEEN ? AND ?)";
  EXPECT_EQ(query.to_sql(), expected_sql);
