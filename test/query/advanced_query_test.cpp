...
TEST_F(AdvancedQueryTest, JoinTest) {
  ...
  auto query = relx::query::select(users.id, users.name, posts.id, posts.title)
                   .from(users)
                   .join(posts, relx::query::on(users.id == posts.user_id));

  // Check the SQL generated
  EXPECT_EQ(query.to_sql(), "SELECT users.id, users.name, posts.id, posts.title FROM users JOIN "
  ...
  std::vector<std::vector<std::string>> rows = {{"1", "John Doe", "101", "First Post"},
  ...
  auto user_posts = results.transform<UserPost>(
      [](const relx::result::Row& row) -> relx::result::ResultProcessingResult<UserPost> {
...
TEST_F(AdvancedQueryTest, WhereClauseTest) {
  ...
  auto query = relx::query::select(users.id, users.name, users.age)
                   .from(users)
                   .where(users.age > 30 && users.is_active == true);

  // Check the SQL generated
  EXPECT_EQ(query.to_sql(), "SELECT users.id, users.name, users.age FROM users WHERE ((users.age > "
                            "?) AND (users.is_active = ?))");
  ...
  std::vector<std::vector<std::string>> rows = {{"3", "Bob Johnson", "45"},
                                                {"5", "Maria Garcia", "38"}};
  std::string raw_results = create_raw_results(headers, rows);
...
TEST_F(AdvancedQueryTest, GroupByTest) {
  ...
  auto query = relx::query::select_expr(users.id, users.name,
  ...
  EXPECT_EQ(
      query.to_sql(),
      "SELECT users.id, users.name, COUNT(posts.id) AS post_count FROM users JOIN posts ON "
      "(users.id = posts.user_id) GROUP BY users.id, users.name HAVING (COUNT(posts.id) > ?)");
  ...
  std::vector<std::vector<std::string>> rows = {{"1", "John Doe", "5"}, {"3", "Bob Johnson", "3"}};
  std::string raw_results = create_raw_results(headers, rows);
  ...
  auto user_post_counts = results.transform<UserPostCount>(
      [](const relx::result::Row& row) -> relx::result::ResultProcessingResult<UserPostCount> {
        ...
        if (!user_id || !name || !post_count) {
          return std::unexpected(
              relx::result::ResultError{"Failed to extract user post count data"});
        }
...
TEST_F(AdvancedQueryTest, ComplexQueryTest) {
  ...
  auto query = relx::query::select_expr(
                   ...
                   .from(departments)
                   .join(users, relx::query::on(departments.id == users.department_id))
                   .join(posts, relx::query::on(users.id == posts.user_id))
                   .join(comments, relx::query::on(posts.id == comments.post_id))
                   .where(departments.budget > 10000.0 && posts.views >= 100)
  ...
  EXPECT_EQ(
      query.to_sql(),
      "SELECT departments.name, COUNT(DISTINCT users.id) AS user_count, COUNT(posts.id) AS "
      "post_count, SUM(posts.views) AS total_views FROM departments JOIN users ON (departments.id "
      "= users.department_id) JOIN posts ON (users.id = posts.user_id) JOIN comments ON (posts.id "
      "= comments.post_id) WHERE ((departments.budget > ?) AND (posts.views >= ?)) GROUP BY "
      "departments.name ORDER BY COUNT(DISTINCT users.id) DESC LIMIT ?");
  ...
  std::vector<std::vector<std::string>> rows = {{"Engineering", "15", "45", "7500"},
                                                {"Sales", "12", "36", "6300"},
                                                {"Marketing", "8", "30", "5200"}};
  std::string raw_results = create_raw_results(headers, rows);
  ...
  auto summaries = results.transform<DepartmentSummary>(
      [](const relx::result::Row& row) -> relx::result::ResultProcessingResult<DepartmentSummary> {
        auto name = row.get<std::string>("name");
        ...
        if (!name || !user_count || !post_count || !total_views) {
          return std::unexpected(
              relx::result::ResultError{"Failed to extract department summary data"});
        }
...
TEST_F(AdvancedQueryTest, PartialColumnSelectionTest) {
  ...
  auto query = relx::query::select(users.id, users.name).from(users).where(users.age > 25);
...
TEST_F(AdvancedQueryTest, LeftJoinWithNullValues) {
  ...
  auto query = relx::query::select(users.id, users.name, posts.id, posts.title)
                   .from(users)
                   .left_join(posts, relx::query::on(users.id == posts.user_id));

  // Check the SQL generated
  EXPECT_EQ(query.to_sql(), "SELECT users.id, users.name, posts.id, posts.title FROM users LEFT "
