...
TEST(UpdateQueryTest, UpdateWithComplexWhere) {
  ...
  EXPECT_EQ(query.to_sql(),
            "UPDATE users SET name = ? WHERE ((users.id > ?) AND (users.active = ?))");
...
TEST(UpdateQueryTest, UpdateWithFunctionInSet) {
  ...
  auto query = query::update(users).set(users.last_login, current_timestamp).where(users.id == 1);

  EXPECT_EQ(query.to_sql(),
            "UPDATE users SET last_login = CURRENT_TIMESTAMP() WHERE (users.id = ?)");
...
//     EXPECT_EQ(query.to_sql(), "UPDATE users SET status = CASE WHEN (login_count > ?) THEN ? WHEN
//     (login_count > ?) THEN ? ELSE ? END WHERE (id = ?)");
...
TEST(UpdateQueryTest, UpdateWithInCondition) {
  ...
  auto query = query::update(users).set(users.active, true).where(query::in(users.id, ids));
...
TEST(UpdateQueryTest, UpdateWithReturning) {
  ...
  EXPECT_EQ(expr_query.to_sql(), "UPDATE users SET name = ?, email = ? WHERE (users.active = ?) "
                                 "RETURNING users.id, COUNT(), users.name AS updated_name");
  ...
  EXPECT_EQ(mixed_query.to_sql(), "UPDATE users SET name = ?, email = ? WHERE (users.active = ?) "
                                  "RETURNING users.id, COUNT(), users.name AS updated_name");
