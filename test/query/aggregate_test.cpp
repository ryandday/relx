...
TEST(AggregateTest, MinMax) {
  ...
  std::string expected_sql =
      "SELECT MIN(users.age) AS youngest, MAX(users.age) AS oldest FROM users";
  EXPECT_EQ(query.to_sql(), expected_sql);
  EXPECT_TRUE(query.bind_params().empty());
}
...
TEST(AggregateTest, MultipleAggregates) {
  ...
  std::string expected_sql = "SELECT COUNT(*) AS total_users, AVG(users.age) AS average_age, "
                             "SUM(users.login_count) AS total_logins FROM users";
  EXPECT_EQ(query.to_sql(), expected_sql);
  EXPECT_TRUE(query.bind_params().empty());
}
...
TEST(AggregateTest, AggregatesWithWhere) {
  ...
  std::string expected_sql = "SELECT COUNT(*) AS active_users, AVG(users.age) AS average_age FROM "
                             "users WHERE (users.is_active = ?)";
  EXPECT_EQ(query.to_sql(), expected_sql);
...
TEST(AggregateTest, SimpleGroupBy) {
  ...
  std::string expected_sql =
      "SELECT users.age, COUNT(*) AS user_count FROM users GROUP BY users.age";
  EXPECT_EQ(query.to_sql(), expected_sql);
  EXPECT_TRUE(query.bind_params().empty());
}
...
TEST(AggregateTest, GroupByMultipleColumns) {
  ...
  std::string expected_sql = "SELECT users.age, users.is_active, COUNT(*) AS user_count FROM users "
                             "GROUP BY users.age, users.is_active";
  EXPECT_EQ(query.to_sql(), expected_sql);
  EXPECT_TRUE(query.bind_params().empty());
}
...
TEST(AggregateTest, GroupByWithHaving) {
  ...
  std::string expected_sql = "SELECT users.age, COUNT(*) AS user_count FROM users GROUP BY "
                             "users.age HAVING (COUNT(*) > ?)";
  EXPECT_EQ(query.to_sql(), expected_sql);
...
TEST(AggregateTest, GroupByWithHavingAndWhere) {
  ...
  std::string expected_sql =
      "SELECT posts.user_id, COUNT(*) AS post_count, SUM(posts.views) AS total_views FROM posts "
      "WHERE (posts.is_published = ?) GROUP BY posts.user_id HAVING (SUM(posts.views) > ?)";
  EXPECT_EQ(query.to_sql(), expected_sql);
...
TEST(AggregateTest, JoinWithGroupBy) {
  ...
  std::string expected_sql =
      "SELECT users.id, users.name, COUNT(posts.id) AS post_count FROM users LEFT JOIN posts ON "
      "(users.id = posts.user_id) GROUP BY users.id, users.name ORDER BY COUNT(posts.id) DESC";
  EXPECT_EQ(query.to_sql(), expected_sql);
  EXPECT_TRUE(query.bind_params().empty());
}