...
TEST_F(DateFunctionTest, DateDiffFunction) {
  ...
  auto query1 = select_expr(date_diff("day", emp.hire_date, emp.birth_date)).from(emp);

  EXPECT_EQ(query1.to_sql(),
            "SELECT (employees.birth_date::date - employees.hire_date::date) FROM employees");
  EXPECT_TRUE(query1.bind_params().empty());

  // Test with different units
  auto query_years = select_expr(date_diff("year", emp.birth_date, current_date())).from(emp);

  EXPECT_EQ(query_years.to_sql(),
            "SELECT EXTRACT(YEAR FROM AGE(CURRENT_DATE, employees.birth_date)) FROM employees");
  ...
  EXPECT_EQ(
      query_hours.to_sql(),
      "SELECT EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - employees.last_review))/3600 FROM employees");
}

// Test 2: DATE_ADD and DATE_SUB functions
TEST_F(DateFunctionTest, DateAddSubFunctions) {
  // Test date addition
  auto add_query = select_expr(date_add(emp.hire_date, interval("1 year"))).from(emp);
  ...
  auto sub_query = select_expr(date_sub(emp.hire_date, interval("6 months"))).from(emp);

  EXPECT_EQ(sub_query.to_sql(),
            "SELECT (employees.hire_date - INTERVAL '6 months') FROM employees");

  // Test with various intervals
  auto day_add = select_expr(date_add(emp.hire_date, interval("30 days"))).from(emp);

  EXPECT_EQ(day_add.to_sql(), "SELECT (employees.hire_date + INTERVAL '30 days') FROM employees");

  auto week_sub = select_expr(date_sub(current_date(), interval("2 weeks"))).from(emp);
...
TEST_F(DateFunctionTest, ExtractFunction) {
  ...
  auto year_query = select_expr(extract("year", emp.birth_date)).from(emp);
  ...
  auto hour_query = select_expr(extract("hour", emp.last_review)).from(emp);

  EXPECT_EQ(hour_query.to_sql(), "SELECT EXTRACT(hour FROM employees.last_review) FROM employees");

  auto minute_query = select_expr(extract("minute", emp.last_review)).from(emp);

  EXPECT_EQ(minute_query.to_sql(),
            "SELECT EXTRACT(minute FROM employees.last_review) FROM employees");

  auto second_query = select_expr(extract("second", emp.last_review)).from(emp);

  EXPECT_EQ(second_query.to_sql(),
            "SELECT EXTRACT(second FROM employees.last_review) FROM employees");
}

// Test 4: DATE_TRUNC function
TEST_F(DateFunctionTest, DateTruncFunction) {
  // Test truncating to year
  auto year_trunc = select_expr(date_trunc("year", emp.hire_date)).from(emp);
  ...
  auto month_trunc = select_expr(date_trunc("month", emp.birth_date)).from(emp);

  EXPECT_EQ(month_trunc.to_sql(),
            "SELECT DATE_TRUNC('month', employees.birth_date) FROM employees");

  // Test truncating to day
  auto day_trunc = select_expr(date_trunc("day", emp.last_review)).from(emp);

  EXPECT_EQ(day_trunc.to_sql(), "SELECT DATE_TRUNC('day', employees.last_review) FROM employees");

  // Test truncating to hour
  auto hour_trunc = select_expr(date_trunc("hour", current_timestamp())).from(emp);
...
TEST_F(DateFunctionTest, HelperFunctions) {
  ...
  auto age_query = select_expr(age_in_years(emp.birth_date)).from(emp);

  EXPECT_EQ(age_query.to_sql(),
            "SELECT EXTRACT(YEAR FROM AGE(CURRENT_DATE, employees.birth_date)) FROM employees");

  // Test days_since helper
  auto days_since_query = select_expr(days_since(emp.hire_date)).from(emp);

  EXPECT_EQ(days_since_query.to_sql(),
            "SELECT (CURRENT_DATE::date - employees.hire_date::date) FROM employees");

  // Test days_until helper
  auto days_until_query = select_expr(days_until(emp.termination_date)).from(emp);

  EXPECT_EQ(days_until_query.to_sql(),
            "SELECT (employees.termination_date::date - CURRENT_DATE::date) FROM employees");

  // Test start_of_year helper
  auto start_year = select_expr(start_of_year(emp.hire_date)).from(emp);

  EXPECT_EQ(start_year.to_sql(), "SELECT DATE_TRUNC('year', employees.hire_date) FROM employees");

  // Test start_of_month helper
  auto start_month = select_expr(start_of_month(emp.hire_date)).from(emp);

  EXPECT_EQ(start_month.to_sql(), "SELECT DATE_TRUNC('month', employees.hire_date) FROM employees");

  // Test start_of_day helper
  auto start_day = select_expr(start_of_day(emp.last_review)).from(emp);
...
TEST_F(DateFunctionTest, ExtractHelperFunctions) {
  // Test year() helper
  auto year_query = select_expr(year(emp.birth_date)).from(emp);

  EXPECT_EQ(year_query.to_sql(), "SELECT EXTRACT(year FROM employees.birth_date) FROM employees");

  // Test month() helper
  auto month_query = select_expr(month(emp.hire_date)).from(emp);

  EXPECT_EQ(month_query.to_sql(), "SELECT EXTRACT(month FROM employees.hire_date) FROM employees");

  // Test day() helper
  auto day_query = select_expr(day(emp.hire_date)).from(emp);

  EXPECT_EQ(day_query.to_sql(), "SELECT EXTRACT(day FROM employees.hire_date) FROM employees");

  // Test day_of_week() helper
  auto dow_query = select_expr(day_of_week(emp.birth_date)).from(emp);

  EXPECT_EQ(dow_query.to_sql(), "SELECT EXTRACT(dow FROM employees.birth_date) FROM employees");

  // Test day_of_year() helper
  auto doy_query = select_expr(day_of_year(emp.birth_date)).from(emp);

  EXPECT_EQ(doy_query.to_sql(), "SELECT EXTRACT(doy FROM employees.birth_date) FROM employees");

  // Test hour() helper
  auto hour_query = select_expr(hour(emp.last_review)).from(emp);

  EXPECT_EQ(hour_query.to_sql(), "SELECT EXTRACT(hour FROM employees.last_review) FROM employees");

  // Test minute() helper
  auto minute_query = select_expr(minute(emp.last_review)).from(emp);

  EXPECT_EQ(minute_query.to_sql(),
            "SELECT EXTRACT(minute FROM employees.last_review) FROM employees");

  // Test second() helper
  auto second_query = select_expr(second(emp.last_review)).from(emp);
...
TEST_F(DateFunctionTest, ComplexDateQueries) {
  ...
  auto complex_query = select(emp.id, emp.name, as(age_in_years(emp.birth_date), "age"),
...
TEST_F(DateFunctionTest, DateFunctionsInWhere) {
  ...
  EXPECT_EQ(recent_hires.to_sql(), "SELECT employees.id, employees.name FROM employees "
  ...
  auto born_in_1990 = select(emp.id, emp.name).from(emp).where(year(emp.birth_date) == 1990);

  EXPECT_EQ(born_in_1990.to_sql(), "SELECT employees.id, employees.name FROM employees "
  ...
  auto has_review = select(emp.id, emp.name).from(emp).where(emp.last_review.is_not_null());

  EXPECT_EQ(has_review.to_sql(), "SELECT employees.id, employees.name FROM employees "
...
TEST_F(DateFunctionTest, DateFunctionsInOrderBy) {
  ...
  auto order_by_age =
      select(emp.id, emp.name).from(emp).order_by(desc(age_in_years(emp.birth_date)));
  ...
  auto order_by_hire =
      select(emp.id, emp.name).from(emp).order_by(desc(emp.hire_date), asc(emp.name));

  EXPECT_EQ(order_by_hire.to_sql(), "SELECT employees.id, employees.name FROM employees "
...
TEST_F(DateFunctionTest, DateFunctionsWithGroupBy) {
  ...
  auto by_hire_year = select_expr(year(emp.hire_date), as(count_all(), "employee_count"))
                          .from(emp)
  ...
  auto by_quarter = select_expr(date_trunc("quarter", emp.hire_date),
...
TEST_F(DateFunctionTest, IntervalExpressions) {
  ...
  auto intervals = select_expr(interval("1 day"), interval("2 weeks"), interval("3 months"),
                               interval("1 year"), interval("5 hours"), interval("30 minutes"))
                       .from(emp);

  EXPECT_EQ(intervals.to_sql(), "SELECT INTERVAL '1 day', INTERVAL '2 weeks', INTERVAL '3 months', "
                                "INTERVAL '1 year', INTERVAL '5 hours', INTERVAL '30 minutes' "
                                "FROM employees");
}
...
TEST_F(DateFunctionTest, OptionalDateColumns) {
  ...
  auto optional_date_query = select_expr(emp.id, emp.name,
                                         as(days_since(emp.last_review), "days_since_review"),
                                         as(date_add(emp.last_review, interval("1 year")),
                                            "next_review_due"))
                                 .from(emp)
  ...
  auto termination_query = select(emp.id, emp.name).from(emp).where(emp.termination_date.is_null());

  EXPECT_EQ(termination_query.to_sql(), "SELECT employees.id, employees.name FROM employees "
...
TEST_F(DateFunctionTest, TypeSafetyCompilation) {
  ...
  auto valid_ops = select_expr(date_diff("day", emp.hire_date, emp.birth_date),
                               extract("year", emp.birth_date),
                               date_add(emp.hire_date, interval("1 year")),
                               age_in_years(emp.birth_date), year(emp.hire_date))
                       .from(emp);
...
TEST_F(DateFunctionTest, DateArithmeticWithCurrentFunctions) {
  ...
  auto future_dates =
      select_expr(as(date_add(current_date(), interval("30 days")), "thirty_days_from_now"),
...
TEST_F(DateFunctionTest, ComplexBusinessLogic) {
  ...
  auto retirement_eligible = select(emp.id, emp.name)
                                 .from(emp)
                                 .where((age_in_years(emp.birth_date) >= 65) &&
...
TEST_F(DateFunctionTest, HelperFunctionsWithExpressions) {
  ...
  auto clean_syntax_query = select_expr(as(year(emp.hire_date), "hire_year"),
                                        as(month(emp.hire_date), "hire_month"),
                                ...
                                .from(emp)
                                .where((year(emp.hire_date) == year(current_date())) &&
  ...
  auto verbose_query = select(emp.id, emp.name)
                           .from(emp)
                           .where(extract("month", emp.hire_date) ==
                                  extract("month", current_date()));

  // New clean way:
  auto clean_query =
      select(emp.id, emp.name).from(emp).where(month(emp.hire_date) == month(current_date()));
...
TEST_F(DateFunctionTest, ComplexNestedDateFunctionCompositions) {
  ...
  auto complex_nested_1 = select(emp.id, emp.name)
                              .from(emp)
                              .where(month(date_add(start_of_year(emp.birth_date),
                                                    interval("6 months"))) == month(emp.hire_date));

  EXPECT_EQ(complex_nested_1.to_sql(), "SELECT employees.id, employees.name FROM employees "
                                       "WHERE (EXTRACT(month FROM (DATE_TRUNC('year', "
                                       "employees.birth_date) + INTERVAL '6 months')) = "
  ...
  auto complex_nested_2 =
      select_expr(emp.id, emp.name,
          ...
          .from(emp)
          .where((month(emp.hire_date) == month(date_sub(current_date(), interval("3 years")))) &&
  ...
  EXPECT_EQ(complex_nested_2.to_sql(),
            ...
            "FROM employees "
            "WHERE ((EXTRACT(month FROM employees.hire_date) = EXTRACT(month FROM (CURRENT_DATE - "
            "INTERVAL '3 years'))) AND "
            "(EXTRACT(year FROM employees.hire_date) >= EXTRACT(year FROM (CURRENT_DATE - INTERVAL "
            "'10 years'))))");
      ...
      select_expr(
          ...
          .where(
              ...
              (date_trunc("quarter", emp.hire_date) ==
               date_trunc("quarter",
                          date_add(start_of_year(emp.birth_date), interval("25 years")))) &&
  ...
  EXPECT_EQ(
      ...
      "DATE_TRUNC('quarter', (DATE_TRUNC('year', employees.birth_date) + INTERVAL '25 years'))) "
      "AND "
  ...
  auto recursive_complex =
      select_expr(
          emp.id, emp.name, as(age_in_years(emp.birth_date), "current_age"),
          as(year(emp.hire_date), "hire_year"),
          as(date_diff(
                 "year",
                 date_trunc("year", date_sub(current_date(),
                                             interval("50 years"))),  // 50 years ago as "epoch"
                 start_of_year(emp.hire_date)),
             "years_since_epoch"))
          .from(emp)
          .where((age_in_years(emp.birth_date) * 2) ==
                 date_diff("year",
  ...
  EXPECT_EQ(recursive_complex.to_sql(),
            ...
            "EXTRACT(year FROM employees.hire_date) AS hire_year, "
            "EXTRACT(YEAR FROM AGE(DATE_TRUNC('year', employees.hire_date), DATE_TRUNC('year', "
            "(CURRENT_DATE - INTERVAL '50 years')))) AS years_since_epoch "
            "FROM employees "
            "WHERE ((EXTRACT(YEAR FROM AGE(CURRENT_DATE, employees.birth_date)) * ?) = "
            "EXTRACT(YEAR FROM AGE(DATE_TRUNC('year', employees.hire_date), DATE_TRUNC('year', "
            "(CURRENT_DATE - INTERVAL '50 years')))))");
  ...
  auto maximum_complexity =
      select_expr(emp.id,
                  as(day(date_add(date_trunc("month", date_sub(start_of_year(current_date()),
                                                               interval("1 year"))),
                                  interval("45 days"))),
                     "complex_day_calculation"))
          .from(emp)
          .where(year(date_add(start_of_month(emp.hire_date), interval("6 months"))) ==
                 year(date_sub(
                     current_date(),
                     interval(
                         // This would be dynamic in real usage, but for test we'll use a constant
                         "2 years"))))
          .order_by(month(date_add(date_trunc("year", emp.birth_date), interval("20 years"))));

  EXPECT_EQ(maximum_complexity.to_sql(),
            "SELECT employees.id, "
            "EXTRACT(day FROM (DATE_TRUNC('month', (DATE_TRUNC('year', CURRENT_DATE) - INTERVAL '1 "
            "year')) + INTERVAL '45 days')) "
            "AS complex_day_calculation "
            "FROM employees "
            "WHERE (EXTRACT(year FROM (DATE_TRUNC('month', employees.hire_date) + INTERVAL '6 "
            "months')) = "
            "EXTRACT(year FROM (CURRENT_DATE - INTERVAL '2 years'))) "
            "ORDER BY EXTRACT(month FROM (DATE_TRUNC('year', employees.birth_date) + INTERVAL '20 "
            "years'))");
}
...
TEST_F(DateFunctionTest, DateArithmeticOperatorOverloads) {
  ...
  auto col_plus_interval = select_expr(emp.id,
  ...
  auto explicit_functions = select_expr(emp.id, as(date_add(emp.hire_date, interval("1 year")),
                                                   "hire_plus_year"))
                                .from(emp);

  auto operator_version =
      select_expr(emp.id, as(emp.hire_date + interval("1 year"), "hire_plus_year")).from(emp);
  ...
  auto multiple_operations =
      select_expr(emp.id,
                  as(year((emp.hire_date + interval("1 year")) - interval("6 months")),
                     "complex_year_calc"),
                  as(month(current_date() - interval("3 years") + interval("6 months")),
                     "complex_month_calc"))
          .from(emp);

  EXPECT_EQ(multiple_operations.to_sql(), "SELECT employees.id, "
                                          "EXTRACT(year FROM ((employees.hire_date + INTERVAL '1 "
                                          "year') - INTERVAL '6 months')) AS complex_year_calc, "
                                          "EXTRACT(month FROM ((CURRENT_DATE - INTERVAL '3 years') "
                                          "+ INTERVAL '6 months')) AS complex_month_calc "
                                          "FROM employees");
}
...
TEST_F(DateFunctionTest, CurrentDateWithAllFunctions) {
  ...
  auto date_diff_tests =
      select_expr(
          ...
          as(date_diff("year", current_date(), emp.termination_date),
  ...
  auto date_arithmetic_tests =
      select_expr(as(date_add(current_date(), interval("1 year")), "current_plus_year"),
  ...
  auto extract_tests = select_expr(as(extract("year", current_date()), "current_year"),
  ...
  EXPECT_EQ(extract_tests.to_sql(), "SELECT "
                                    "EXTRACT(year FROM CURRENT_DATE) AS current_year, "
                                    "EXTRACT(month FROM CURRENT_DATE) AS current_month, "
                                    "EXTRACT(day FROM CURRENT_DATE) AS current_day, "
  ...
  auto date_trunc_tests =
      select_expr(as(date_trunc("year", current_date()), "current_truncated_to_year"),
  ...
  auto extract_helper_tests = select_expr(as(year(current_date()), "current_year_helper"),
  ...
  auto helper_diff_tests =
      select_expr(as(age_in_years(current_date()), "age_of_current_date"),  // Should be 0
  ...
  auto complex_nested_current =
      select_expr(as(year(date_add(start_of_year(current_date()), interval("6 months"))),
                     "mid_year_of_current"),
                  as(month(date_sub(start_of_month(current_date()), interval("1 day"))),
                     "last_month_from_current"),
                  as(day(date_trunc("week", current_date()) + interval("3 days")),
                     "wednesday_of_current_week"),
                  as(extract("quarter", current_date() + interval("3 months")),
                     "next_quarter_from_current"))
          .from(emp);

  EXPECT_EQ(
      complex_nested_current.to_sql(),
      "SELECT "
      "EXTRACT(year FROM (DATE_TRUNC('year', CURRENT_DATE) + INTERVAL '6 months')) AS "
      "mid_year_of_current, "
      "EXTRACT(month FROM (DATE_TRUNC('month', CURRENT_DATE) - INTERVAL '1 day')) AS "
      "last_month_from_current, "
      "EXTRACT(day FROM (DATE_TRUNC('week', CURRENT_DATE) + INTERVAL '3 days')) AS "
      "wednesday_of_current_week, "
  ...
  EXPECT_EQ(where_clause_tests.to_sql(),
            "SELECT employees.id, employees.name FROM employees "
            "WHERE ((((((((EXTRACT(year FROM employees.hire_date) <= EXTRACT(year FROM "
            "CURRENT_DATE)) AND "
  ...
  auto order_by_tests = select(emp.id, emp.name)
                            .from(emp)
                            .order_by(desc(date_diff("day", emp.hire_date, current_date())),
  ...
  auto group_by_tests =
      select_expr(year(current_date()), month(current_date()), as(count_all(), "total_employees"),
          ...
          .from(emp)
          .group_by(year(current_date()), month(current_date()))
          .having(count_all() > 0 && avg(date_diff("year", emp.birth_date, current_date())) >= 18);
  ...
  EXPECT_EQ(group_by_tests.to_sql(),
            ...
            "HAVING ((COUNT(*) > ?) AND (AVG(EXTRACT(YEAR FROM AGE(CURRENT_DATE, "
