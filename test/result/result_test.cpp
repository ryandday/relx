...
TEST_F(ResultTest, Transformation) {
  ...
  auto users = results.transform<UserData>(
      [](const relx::result::Row& row) -> relx::result::ResultProcessingResult<UserData> {
        auto id = row.get<int>("id");
...
TEST_F(ResultTest, StructuredBinding) {
  ...
  for (const auto& [id, name, age] :
       results.as<int, std::string, int>(std::array<std::string, 3>{"id", "name", "age"})) {
...
TEST_F(ResultTest, StructuredBindingWithCustomIndices) {
  ...
  for (const auto& [id, name, active] :
       results.as<int, std::string, bool>(std::array<size_t, 3>{0, 1, 4})) {
...
TEST_F(ResultTest, StructuredBindingWithColumnNames) {
  ...
  for (const auto& [age, score, email] :
       results.as<int, double, std::string>(std::array<std::string, 3>{"age", "score", "email"})) {
