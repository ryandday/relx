...
TEST_F(LazyParsingTest, BooleanConversion) {
  std::string test_data = "bool_col|numeric_col\n"
                          "true|1\n"
                          "false|0\n";
...
TEST_F(LazyParsingTest, PerformanceComparison) {
  ...
  for (int i = 1; i <= 1000; ++i) {
    large_data += std::to_string(i) + "|User" + std::to_string(i) + "|user" + std::to_string(i) +
