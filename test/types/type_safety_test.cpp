...
TEST(TypeSafetyTest, ValidComparisons) {
    ...
    auto query1 = relx::query::select(t.id)
        .from(t)
        .where(t.id == 42);  // int column with int value
    
    auto query2 = relx::query::select(t.price)
        .from(t)
        .where(t.price > 10.5);  // double column with double value
    
    auto query3 = relx::query::select(t.name)
        .from(t)
        .where(t.name == "test");  // string column with string literal
    
    auto query4 = relx::query::select(t.name)
        .from(t)
        .where(t.name == std::string("test"));  // string column with std::string
    ...
    auto query6 = relx::query::select(t.name)
        .from(t) 
        .where(t.name == test_str);
    
    std::string_view test_sv = "test";
    auto query7 = relx::query::select(t.name)
        .from(t)
        .where(t.name == test_sv);
...
TEST(TypeSafetyTest, OptionalTypeComparisons) {
    ...
    auto query3 = relx::query::select(t.id)
        .from(t)
        .where(t.id == opt_int);  // int with optional<int>
    ...
    auto query7 = relx::query::select(t.name)
        .from(t)
        .where(t.name == opt_str);  // string with optional<string>
...
TEST(TypeSafetyTest, CrossTypeComparisons) {
    ...
    auto query3 = relx::query::select(t.name)
        .from(t)
        .where(t.name == "42");  // string column with string value that looks like a number
    
    // Only test the valid ones
    EXPECT_NO_THROW({
        auto sql3 = query3.to_sql();
    });
}
...
TEST(TypeSafetyTest, AggregateFunctionTypeChecking) {
    ...
    auto valid_sum = relx::query::select_expr(relx::query::sum(t.id))
        .from(t);  // int column
    
    auto valid_avg = relx::query::select_expr(relx::query::avg(t.price))
        .from(t);  // double column
    
    auto valid_min_max = relx::query::select_expr(
        relx::query::min(t.id),
    ...
    auto valid_lower = relx::query::select_expr(relx::query::lower(t.name))
        .from(t);  // string column
...
TEST(TypeSafetyTest, ArithmeticOperationsTypeChecking) {
    ...
    auto valid_addition = relx::query::select_expr(t.id + t.optional_id)
        .from(t);  // int + optional<int>
        
    auto valid_price_calc = relx::query::select_expr(t.price * 1.2)
        .from(t);  // double * double
        
    auto valid_subtraction = relx::query::select_expr(t.price - 10.0)
        .from(t);  // double - double
        
    auto valid_division = relx::query::select_expr(t.id / 2)
        .from(t);  // int / int
...
TEST(TypeSafetyTest, OrderByTypeChecking) {
    ...
    auto valid_order1 = relx::query::select(t.id, t.name)
        .from(t)
        .order_by(t.id);  // int column
        
    auto valid_order2 = relx::query::select(t.name, t.price)
        .from(t)
        .order_by(t.name);  // string column
        
    auto valid_order3 = relx::query::select(t.price)
        .from(t)
        .order_by(t.price);  // double column
