...
struct Item {
  ...
  table_check_constraint<"(price < 100.0 AND category = 'books') OR category != 'books'">
      books_pricing;
};
...
TEST(CheckConstraintTest, TableLevelConstraints) {
  ...
  EXPECT_EQ(category_check.sql_definition(),
  ...
  auto electronics_price =
      table_check<"(price <= 1000.0 AND category = 'electronics') OR category != 'electronics'">();
...
TEST(CheckConstraintTest, TableWithCheckConstraints) {
  ...
  EXPECT_TRUE(create_sql.find("CHECK (category IN ('electronics', 'books', 'clothing'))") !=
              std::string::npos);
  EXPECT_TRUE(
      create_sql.find("CHECK ((price < 100.0 AND category = 'books') OR category != 'books')") !=
      std::string::npos);
}
...
TEST(CheckConstraintTest, SpecialCharacters) {
  ...
  auto backslash_check =
      table_check<"item_name LIKE '%\\special\\%' OR item_name LIKE '%\"quoted\"%'">();
  EXPECT_EQ(backslash_check.sql_definition(),
            "CHECK (item_name LIKE '%\\special\\%' OR item_name LIKE '%\"quoted\"%')");

  // Test with comparison operators and parentheses
  auto complex_check =
      table_check<"(price > 100.0 AND price <= 1000.0) OR (price = 50.0 AND category = 'sale')">();
  ...
  auto special_name_check =
      table_check<"item_name LIKE '%O''Brien''s%' OR item_name LIKE '%100\\%%'">();
