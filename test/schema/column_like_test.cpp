...
TEST(ColumnLikeTest, LikeMethod) {
  ...
  auto query = relx::query::select(u.id, u.name).from(u).where(u.email.like("%admin%"));
...
TEST(ColumnLikeTest, IsNullMethod) {
  ...
  auto query = relx::query::select(u.id, u.name).from(u).where(u.email.is_null());
...
TEST(ColumnLikeTest, IsNotNullMethod) {
  ...
  auto query = relx::query::select(u.id, u.name).from(u).where(u.email.is_not_null());
...
TEST(ColumnLikeTest, CombinedConditions) {
  ...
  auto query = relx::query::select(u.id, u.name)
                   .from(u)
                   .where((u.age >= 18 && u.is_active == true) || u.email.like("%admin%"));

  std::string expected_sql = "SELECT users.id, users.name FROM users WHERE (((users.age >= ?) AND "
                             "(users.is_active = ?)) OR users.email LIKE ?)";
  EXPECT_EQ(query.to_sql(), expected_sql);
