...
// static_assert(ColumnTypeConcept<ValidColumnType>, "ValidColumnType should satisfy
// ColumnTypeConcept"); Commenting out tests that don't match actual implementation
// static_assert(!ColumnTypeConcept<InvalidColumnType>, "InvalidColumnType should not satisfy
// ColumnTypeConcept"); static_assert(!ColumnTypeConcept<void>, "void should not satisfy
// ColumnTypeConcept"); static_assert(!ColumnTypeConcept<std::vector<int>>, "std::vector<int> should
// not satisfy ColumnTypeConcept");

// is_column concept tests
// static_assert(is_column<column<"id", int>>, "column<id, int> should satisfy is_column");
// static_assert(is_column<nullable_column<"name", std::string>>, "nullable_column<name,
// std::string> should satisfy is_column"); static_assert(!is_column<int>, "int should not satisfy
// is_column"); static_assert(!is_column<std::string>, "std::string should not satisfy is_column");

// is_constraint concept tests
// static_assert(is_constraint<primary_key<&ValidTable::id>>, "primary_key should satisfy
// is_constraint"); Commenting out tests that don't match actual implementation
// static_assert(is_constraint<relx::schema::index<&ValidTable::id>>, "index should satisfy
// is_constraint"); static_assert(!is_constraint<column<"id", int>>, "column should not satisfy
// is_constraint"); static_assert(!is_constraint<nullable_column<"name", std::string>>,
// "nullable_column should not satisfy is_constraint");
...
struct custom_column {
  ...
  std::string sql_definition() const {
    return std::string(std::string_view(name)) + " " + std::string(std::string_view(sql_type)) +
           " CUSTOM";
  }
};
