...
struct Post {
  ...
  foreign_key<&Post::category_id, &Category::id> category_fk{reference_action::set_null,
...
TEST(ForeignKeyTest, ForeignKeyWithActions) {
  ...
  EXPECT_EQ(
      post.category_fk.sql_definition(),
      "FOREIGN KEY (category_id) REFERENCES categories(id) ON DELETE SET NULL ON UPDATE CASCADE");

  // Test that the actions appear in the CREATE TABLE statement
  std::string sql = create_table(post).to_sql();
  EXPECT_TRUE(sql.find("FOREIGN KEY (category_id) REFERENCES categories(id) ON DELETE SET NULL ON "
                       "UPDATE CASCADE") != std::string::npos);
}
...
TEST(ForeignKeyTest, MultipleForeignKeys) {
  ...
  std::string constraints = collect_constraint_definitions(post);
  EXPECT_TRUE(constraints.find("FOREIGN KEY (user_id) REFERENCES users(id)") != std::string::npos);
  EXPECT_TRUE(constraints.find("FOREIGN KEY (category_id) REFERENCES categories(id)") !=
              std::string::npos);
...
TEST(ForeignKeyTest, ReferenceActions) {
  ...
  EXPECT_EQ(std::string_view(reference_action_to_string(reference_action::set_default)),
            "SET DEFAULT");
  ...
  foreign_key<&Post::user_id, &User::id> cascade_fk{reference_action::cascade,
                                                    reference_action::cascade};
  EXPECT_EQ(cascade_fk.sql_definition(),
            "FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE ON UPDATE CASCADE");

  foreign_key<&Post::user_id, &User::id> restrict_fk{reference_action::restrict,
                                                     reference_action::no_action};
  EXPECT_EQ(restrict_fk.sql_definition(),
            "FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE RESTRICT");

  foreign_key<&Post::user_id, &User::id> set_default_fk{reference_action::no_action,
                                                        reference_action::set_default};
