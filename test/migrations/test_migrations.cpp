...
struct UsersV2 {
    ...
    column<UsersV2, "created_at", std::string, 
                 string_default<"CURRENT_TIMESTAMP", true>> created_at;  // New column with default
...
struct TableWithDefaults {
    ...
    column<TableWithDefaults, "status", std::string, 
           string_default<"active", false>> status;  // Added default
};
...
TEST(MigrationsTest, TestDropConstraintOperations) {
    ...
    if (drop_constraint_migration.size() > 0) {
        ...
        EXPECT_EQ(drop_forward[0], "ALTER TABLE constraint_test DROP CONSTRAINT constraint_test_unique_1;");
        EXPECT_EQ(drop_forward[1], "ALTER TABLE constraint_test DROP CONSTRAINT constraint_test_unique_0;");
...
TEST(MigrationsTest, TestIndexOperations) {
    ...
    if (add_index_migration.size() > 0 && drop_index_migration.size() > 0) {
        ...
        std::cout << "❓ ADD_INDEX/DROP_INDEX: No explicit index operations (using constraint-based indexes)" << std::endl;
...
struct NewEmployeeTable {
    ...
    column<NewEmployeeTable, "phone_number", std::optional<std::string>> phone_number; // renamed from phone
};
...
TEST(MigrationsTest, TestColumnRenaming) {
    ...
    EXPECT_EQ(forward_with_mappings[0], "ALTER TABLE employees RENAME COLUMN last_name TO family_name;");
    ...
    EXPECT_EQ(forward_with_mappings[3], "ALTER TABLE employees RENAME COLUMN first_name TO given_name;");
    
    // Test exact rollback SQL (reverse order)
    EXPECT_EQ(rollback_with_mappings[0], "ALTER TABLE employees RENAME COLUMN given_name TO first_name;");
    EXPECT_EQ(rollback_with_mappings[1], "ALTER TABLE employees RENAME COLUMN email TO email_addr;");
    EXPECT_EQ(rollback_with_mappings[2], "ALTER TABLE employees RENAME COLUMN phone_number TO phone;");
    EXPECT_EQ(rollback_with_mappings[3], "ALTER TABLE employees RENAME COLUMN family_name TO last_name;");
...
TEST(MigrationsTest, TestBidirectionalTransformations) {
    ...
    EXPECT_EQ(forward_sql[0], "ALTER TABLE products ADD COLUMN price_dollars TEXT NOT NULL;");
    EXPECT_EQ(forward_sql[1], "UPDATE products SET price_dollars = CAST(price_cents / 100.0 AS TEXT) || ' USD';");
    ...
    EXPECT_EQ(rollback_sql[1], "UPDATE products SET price_cents = CAST(REPLACE(price_dollars, ' USD', '') AS DECIMAL) * 100;");
