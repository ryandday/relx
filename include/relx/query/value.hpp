...
public:
  ...
  constexpr explicit Value(std::optional<T> value) : value_(std::move(value)) {}
...
public:
  ...
  constexpr explicit Value(boost::uuids::uuid value) : value_(value) {}
  ...
  constexpr std::vector<bind_param> bind_params() const override {
    ...
    param.binary.assign(value_.begin(), value_.end());  // 16 bytes verbatim
    return {param};
  }
...
public:
  ...
  constexpr explicit Value(std::string value) : value_(std::move(value)) {}

  constexpr std::string to_sql() const override { return "?"; }

  constexpr std::vector<bind_param> bind_params() const override { return {value_}; }
...
public:
  ...
  constexpr explicit Value(std::string_view value) : value_(value) {}

  constexpr std::string to_sql() const override { return "?"; }

  constexpr std::vector<bind_param> bind_params() const override { return {std::string(value_)}; }
...
public:
  ...
  constexpr explicit Value(const char* value) : value_(value) {}
...
inline constexpr auto val(const char* str) {
...
inline constexpr auto val(std::string str) {
...
/// @return A Value<std::string_view> expression
inline constexpr auto val(std::string_view sv) {
...
inline constexpr auto val(boost::uuids::uuid u) {
...
inline constexpr auto val(int i) {
  return Value<int>(i);
}
...
inline constexpr auto val(long l) {
  return Value<long>(l);
}
...
inline constexpr auto val(long long ll) {
...
inline constexpr auto val(double d) {
...
inline constexpr auto val(float f) {
...
inline constexpr auto val(bool b) {
  return Value<bool>(b);
}
...
template <typename T>
constexpr auto val(std::optional<T> opt) {
...
public:
  ...
  constexpr explicit Value(std::nullopt_t) {}

  constexpr std::string to_sql() const override { return "?"; }

  constexpr std::vector<bind_param> bind_params() const override { return {bind_param::null()}; }
};

/// @brief Helper to create a value expression from a nullopt
/// @return A Value<std::nullopt_t> expression
inline constexpr auto val(std::nullopt_t) {
