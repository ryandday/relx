...
// ArithmeticExpr-operand operators live only in relx::query: an ArithmeticExpr
// operand always makes relx::query an associated namespace, so ADL finds them, and
// duplicating them here made every such call ambiguous once a schema column was
