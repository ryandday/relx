...
namespace arithmetic_checking {
    ...
    inline constexpr std::string_view arithmetic_error_message = 
        "Arithmetic operations (+, -, *, /) can only be performed on numeric columns (int, long, float, double, etc.). "
...
auto operator+(const schema::column<TableT1, Name1, T1, Modifiers1...>& left,
    ...
    return ArithmeticExpr<decltype(left_expr), decltype(right_expr)>(
        std::move(left_expr), "+", std::move(right_expr));
}

/// @brief Addition operator for column with value
template <typename TableT, schema::FixedString Name, typename T, typename... Modifiers, typename ValueT>
...
auto operator+(const schema::column<TableT, Name, T, Modifiers...>& left, ValueT&& value) {
    ...
    return ArithmeticExpr<decltype(left_expr), decltype(value_expr)>(
        std::move(left_expr), "+", std::move(value_expr));
}

/// @brief Addition operator for value with column (reversed)
template <typename ValueT, typename TableT, schema::FixedString Name, typename T, typename... Modifiers>
requires std::is_arithmetic_v<std::remove_cvref_t<ValueT>>
...
auto operator-(const schema::column<TableT, Name, T, Modifiers...>& left, ValueT&& value) {
    ...
    static_assert(arithmetic_checking::Arithmetic<LeftBaseType>,
                  arithmetic_checking::arithmetic_error_message);
    static_assert(arithmetic_checking::Arithmetic<ValueT>,
                  "Arithmetic operations require numeric values. Cannot subtract non-numeric values from columns.");
...
auto operator-(ValueT&& value, const schema::column<TableT, Name, T, Modifiers...>& right) {
    ...
    auto value_expr = val(std::forward<ValueT>(value));
    auto right_expr = to_expr(right);
    return ArithmeticExpr<decltype(value_expr), decltype(right_expr)>(
...
auto operator*(const schema::column<TableT, Name, T, Modifiers...>& left, ValueT&& value) {
    ...
    static_assert(arithmetic_checking::Arithmetic<LeftBaseType>,
                  arithmetic_checking::arithmetic_error_message);
    static_assert(arithmetic_checking::Arithmetic<ValueT>,
                  "Arithmetic operations require numeric values. Cannot multiply columns by non-numeric values.");
...
auto operator+(const ArithmeticExpr<Left, Right>& left_expr, const schema::column<TableT, Name, T, Modifiers...>& right) {
    ...
    auto right_expr = to_expr(right);
    return ArithmeticExpr<ArithmeticExpr<Left, Right>, decltype(right_expr)>(
        left_expr, "+", std::move(right_expr));
}

/// @brief Addition operator for column with ArithmeticExpr
template <typename TableT, schema::FixedString Name, typename T, typename... Modifiers, SqlExpr Left, SqlExpr Right>
auto operator+(const schema::column<TableT, Name, T, Modifiers...>& left, const ArithmeticExpr<Left, Right>& right_expr) {
    ...
    auto left_expr = to_expr(left);
    return ArithmeticExpr<decltype(left_expr), ArithmeticExpr<Left, Right>>(
...
auto operator+(const ArithmeticExpr<Left, Right>& left_expr, ValueT&& value) {
    ...
    auto value_expr = val(std::forward<ValueT>(value));
    return ArithmeticExpr<ArithmeticExpr<Left, Right>, decltype(value_expr)>(
...
auto operator-(ValueT&& value, const ArithmeticExpr<Left, Right>& right_expr) {
    ...
    auto value_expr = val(std::forward<ValueT>(value));
    return ArithmeticExpr<decltype(value_expr), ArithmeticExpr<Left, Right>>(
...
auto operator+(const column<TableT1, Name1, T1, Modifiers1...>& left,
    ...
    return query::ArithmeticExpr<decltype(left_expr), decltype(right_expr)>(
        std::move(left_expr), "+", std::move(right_expr));
}
...
auto operator+(const column<TableT, Name, T, Modifiers...>& left, ValueT&& value) {
    ...
    auto left_expr = query::to_expr(left);
    auto value_expr = query::val(std::forward<ValueT>(value));
    return query::ArithmeticExpr<decltype(left_expr), decltype(value_expr)>(
        std::move(left_expr), "+", std::move(value_expr));
}
...
/// @brief Addition operator for ArithmeticExpr with column
template <query::SqlExpr Left, query::SqlExpr Right, typename TableT, FixedString Name, typename T, typename... Modifiers>
auto operator+(const query::ArithmeticExpr<Left, Right>& left_expr, const column<TableT, Name, T, Modifiers...>& right) {
    using RightBaseType = query::arithmetic_checking::remove_optional_t<T>;
    static_assert(query::arithmetic_checking::Arithmetic<RightBaseType>,
                  query::arithmetic_checking::arithmetic_error_message);
...
/// @brief Addition operator for column with ArithmeticExpr
template <typename TableT, FixedString Name, typename T, typename... Modifiers, query::SqlExpr Left, query::SqlExpr Right>
auto operator+(const column<TableT, Name, T, Modifiers...>& left, const query::ArithmeticExpr<Left, Right>& right_expr) {
    using LeftBaseType = query::arithmetic_checking::remove_optional_t<T>;
    static_assert(query::arithmetic_checking::Arithmetic<LeftBaseType>,
                  query::arithmetic_checking::arithmetic_error_message);
