...
TableMetadata extract_table_metadata(const Table& table_instance) {
    ...
    boost::pfr::for_each_field(table_instance, [&](const auto& field) {
        ...
        if constexpr (schema::is_column<field_type>) {
            ...
            } else if (sql_def.find("FOREIGN KEY") != std::string::npos || sql_def.find("REFERENCES") != std::string::npos) {
                constraint_meta.type = "FOREIGN_KEY";
                constraint_meta.name = metadata.table_name + "_fk_" + std::to_string(metadata.constraints.size());
            } else if (sql_def.find("UNIQUE") != std::string::npos) {
                constraint_meta.type = "UNIQUE";
                constraint_meta.name = metadata.table_name + "_unique_" + std::to_string(metadata.constraints.size());
            } else if (sql_def.find("CHECK") != std::string::npos) {
                constraint_meta.type = "CHECK";
                constraint_meta.name = metadata.table_name + "_check_" + std::to_string(metadata.constraints.size());
            } else if (sql_def.find("INDEX") != std::string::npos) {
                constraint_meta.type = "INDEX";
                constraint_meta.name = metadata.table_name + "_idx_" + std::to_string(metadata.constraints.size());
            } else {
                constraint_meta.type = "UNKNOWN";
                constraint_meta.name = metadata.table_name + "_constraint_" + std::to_string(metadata.constraints.size());
            }
...
Migration diff_tables(const TableMetadata& old_metadata, const TableMetadata& new_metadata, const MigrationOptions& options = {}) {
    ...
    for (const auto& [old_name, new_name] : options.column_mappings) {
        ...
        if (old_it != old_metadata.columns.end() && new_it != new_metadata.columns.end()) {
            ...
            if (type_changed || nullable_changed || old_core_def != new_core_def) {
                ...
                // This requires data transformation, so we use: ADD new column, UPDATE data, DROP old column
                // This preserves data through transformation
                
                migration.add_operation<AddColumnOperation<ColumnMetadata>>(
                    new_metadata.table_name, new_it->second);
                ...
                if (transform_it != options.column_transformations.end()) {
                    ...
                    migration.add_operation<UpdateDataOperation>(
                        new_metadata.table_name, new_name, old_name, forward_sql, backward_sql);
                } else {
                ...
                migration.add_operation<DropColumnOperation<ColumnMetadata>>(
                    old_metadata.table_name, old_it->second);
            } else {
                // Column is only being renamed (no type/constraint changes)
                // Safe to use RENAME COLUMN operation
                migration.add_operation<RenameColumnOperation>(
                    old_metadata.table_name, old_name, new_name);
            }
        }
    }
    ...
    for (const auto& [col_name, col_meta] : new_metadata.columns) {
        if (processed_new_columns.find(col_name) == processed_new_columns.end() &&
            ...
            migration.add_operation<AddColumnOperation<ColumnMetadata>>(
                new_metadata.table_name, col_meta);
        }
    }
    ...
    for (const auto& [col_name, col_meta] : old_metadata.columns) {
        if (processed_old_columns.find(col_name) == processed_old_columns.end() &&
            ...
            migration.add_operation<DropColumnOperation<ColumnMetadata>>(
                old_metadata.table_name, col_meta);
        }
    }
        ...
        if (processed_new_columns.find(col_name) == processed_new_columns.end()) {
            ...
            if (old_it != old_metadata.columns.end() && old_it->second != new_col_meta) {
                ...
                migration.add_operation<DropColumnOperation<ColumnMetadata>>(
                    old_metadata.table_name, old_it->second);
                migration.add_operation<AddColumnOperation<ColumnMetadata>>(
                    new_metadata.table_name, new_col_meta);
            }
        }
    }
    ...
    for (const auto& [constraint_name, constraint_meta] : new_metadata.constraints) {
        if (old_metadata.constraints.find(constraint_name) == old_metadata.constraints.end()) {
            ...
            migration.add_operation<AddConstraintOperation>(
                new_metadata.table_name, constraint_meta);
        }
    }
    ...
    for (const auto& [constraint_name, constraint_meta] : old_metadata.constraints) {
        if (new_metadata.constraints.find(constraint_name) == new_metadata.constraints.end()) {
            ...
            migration.add_operation<DropConstraintOperation>(
                old_metadata.table_name, constraint_meta);
        }
    }
    ...
    for (const auto& [constraint_name, new_constraint_meta] : new_metadata.constraints) {
        ...
        if (old_it != old_metadata.constraints.end() && old_it->second != new_constraint_meta) {
            ...
            migration.add_operation<DropConstraintOperation>(
                old_metadata.table_name, old_it->second);
            migration.add_operation<AddConstraintOperation>(
                new_metadata.table_name, new_constraint_meta);
        }
    }
...
Migration generate_migration(const OldTable& old_table, const NewTable& new_table, const MigrationOptions& options = {}) {
