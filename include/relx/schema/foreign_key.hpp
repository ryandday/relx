...
constexpr std::string_view reference_action_to_string(reference_action action) {
  switch (action) {
  case reference_action::cascade:
    return "CASCADE";
  case reference_action::restrict:
    return "RESTRICT";
  case reference_action::set_null:
    return "SET NULL";
  case reference_action::set_default:
    return "SET DEFAULT";
  case reference_action::no_action:
    return "NO ACTION";
  default:
    return "NO ACTION";
  }
...
struct fk_impl<
    std::tuple<std::integral_constant<decltype(LocalColumns), LocalColumns>...>,
    std::tuple<std::integral_constant<decltype(ReferencedColumns), ReferencedColumns>...>> {
  using type = std::conditional_t<sizeof...(LocalColumns) == 1,
