...
PostgreSQLStreamingSource::PostgreSQLStreamingSource(PostgreSQLConnection& connection, std::string sql,
...
PostgreSQLStreamingSource::PostgreSQLStreamingSource(PostgreSQLConnection& connection, std::string sql,
    ...
    : connection_(connection), sql_(std::move(sql)), params_(std::move(params)), 
      is_binary_(std::move(is_binary)), use_binary_(true), initialized_(false), 
      finished_(false), convert_bytea_(true), query_active_(false) {}
...
PostgreSQLStreamingSource::PostgreSQLStreamingSource(PostgreSQLStreamingSource&& other) noexcept
    : connection_(other.connection_), sql_(std::move(other.sql_)), params_(std::move(other.params_)),
      is_binary_(std::move(other.is_binary_)), use_binary_(other.use_binary_),
...
PostgreSQLStreamingSource& PostgreSQLStreamingSource::operator=(PostgreSQLStreamingSource&& other) noexcept {
  if (this != &other) {
...
ConnectionResult<void> PostgreSQLStreamingSource::start_query() {
  if (!connection_.is_connected()) {
    return std::unexpected(ConnectionError{.message = "Not connected to database", .error_code = -1});
  }
...
std::string PostgreSQLStreamingSource::convert_pg_bytea_to_binary(const std::string& hex_value) const {
