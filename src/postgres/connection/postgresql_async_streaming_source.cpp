...
PostgreSQLAsyncStreamingSource::PostgreSQLAsyncStreamingSource(PostgreSQLAsyncConnection& connection, 
                                                               std::string sql,
                                                               std::vector<std::string> params)
    : connection_(connection), sql_(std::move(sql)), params_(std::move(params)),
      initialized_(false), finished_(false), convert_bytea_(false), 
      query_active_(false), current_result_(nullptr, PQclear), 
      current_row_index_(0), has_pending_results_(false) {}

PostgreSQLAsyncStreamingSource::~PostgreSQLAsyncStreamingSource() {
  cleanup();
}

PostgreSQLAsyncStreamingSource::PostgreSQLAsyncStreamingSource(PostgreSQLAsyncStreamingSource&& other) noexcept
    : connection_(other.connection_), sql_(std::move(other.sql_)), params_(std::move(other.params_)),
      column_names_(std::move(other.column_names_)), is_bytea_column_(std::move(other.is_bytea_column_)),
      initialized_(other.initialized_), finished_(other.finished_), convert_bytea_(other.convert_bytea_),
      query_active_(other.query_active_), first_row_cached_(std::move(other.first_row_cached_)),
      current_result_(std::move(other.current_result_)), current_row_index_(other.current_row_index_),
...
PostgreSQLAsyncStreamingSource& PostgreSQLAsyncStreamingSource::operator=(PostgreSQLAsyncStreamingSource&& other) noexcept {
  if (this != &other) {
    cleanup();
...
boost::asio::awaitable<ConnectionResult<void>> PostgreSQLAsyncStreamingSource::start_query() {
  if (!connection_.is_connected()) {
    co_return std::unexpected(ConnectionError{.message = "Not connected to database", .error_code = -1});
  }
  ...
  try {
    ...
    if (params_.empty()) {
      ...
      result_code = PQsendQueryParams(pg_conn, pg_sql.c_str(),
    ...
    if (result_code != 1) {
      co_return std::unexpected(ConnectionError{
          .message = std::string("Failed to send query: ") + PQerrorMessage(pg_conn),
          .error_code = -1});
    }
    ...
    if (PQsetSingleRowMode(pg_conn) != 1) {
      co_return std::unexpected(ConnectionError{
          .message = "Failed to enable single-row mode",
          .error_code = -1});
    }
    ...
    while (true) {
      ...
      if (!PQisBusy(pg_conn)) {
        ...
        if (status == PGRES_SINGLE_TUPLE) {
          ...
          PQclear(first_result);
          co_return std::unexpected(ConnectionError{
              .message = std::string("Query execution failed: ") + error_msg,
    ...
    co_return std::unexpected(ConnectionError{.message = std::string("Failed to start streaming query: ") + e.what(), .error_code = -1});
  }
}
...
std::string PostgreSQLAsyncStreamingSource::format_single_row(PGresult* pg_result) {
  ...
  for (int col = 0; col < column_count; ++col) {
    ...
    if (PQgetisnull(pg_result, 0, col)) {
      ...
      if (col < static_cast<int>(is_bytea_column_.size()) && is_bytea_column_[col] && convert_bytea_) {
...
std::string PostgreSQLAsyncStreamingSource::convert_pg_bytea_to_binary(const std::string& hex_value) const {
