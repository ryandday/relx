...
ConnectionResult<result::ResultSet> PostgreSQLStatement::execute(const std::vector<std::string>& params) {
    if (!is_valid_) {
    ...
    if (!params.empty()) {
        ...
        for (size_t i = 0; i < params.size(); ++i) {
            ...
            // Numeric value
            else if (std::all_of(params[i].begin(), params[i].end(), 
                                [](char c) { return std::isdigit(c) || c == '.' || c == '-' || c == '+' || c == 'e' || c == 'E'; })) {
                execute_sql << params[i];
            }
            // Boolean value
            else if (params[i] == "t" || params[i] == "f" || params[i] == "true" || params[i] == "false") {
                execute_sql << params[i];
            }
...
ConnectionResult<PGresult*> PostgreSQLStatement::execute_raw(
    ...
    return std::unexpected(ConnectionError{
        .message = "Direct PGconn access not available through statement interface",
        .error_code = -1
    });
}
...
ConnectionResult<result::ResultSet> PostgreSQLStatement::process_result(PGresult* pg_result) {
    ...
    return std::unexpected(ConnectionError{
        .message = "Direct result processing not needed in statement interface",
        .error_code = -1
    });
}
