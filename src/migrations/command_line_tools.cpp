...
void write_migration_to_file(const Migration& migration, const std::string& filename, bool include_rollback) {
    ...
    if (include_rollback) {
        ...
        if (!rollback_result) {
            throw std::runtime_error("Failed to generate rollback SQL: " + rollback_result.error().format());
        }
